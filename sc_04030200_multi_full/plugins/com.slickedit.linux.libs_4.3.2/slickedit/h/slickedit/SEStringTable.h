...
SEStringTable<T>::insertItem(SEPrefixTree *t, const char *key, size_t len, const T &v)
{
   ...
   if (mCaseSensitive) {
      ...
      vsUTF8Upcase(mUpcaseKey.get_nonconst(), len);
      key = mUpcaseKey.get();
   } else {
      mUpcaseKey.set(key, len);
      mUpcaseKey.upcase();
...
{
   SEPrefixTree *t = searchItem(mPrefixRoot, key.get(), key.length());
   if (t == NULL) {
      return NULL;
   }
   return t->mpData;
...
{
   const SEPrefixTree *t = searchItem(mPrefixRoot, key.get(), key.length());
...
int SEStringTable<T>::remove(const slickedit::SEString &key)
{
   ...
   int status = removeItem(mPrefixRoot, key.get(), key.length());
...
{
   int status = insertItem(mPrefixRoot, key.get(),key.length(), item);
   if (status <= 0) return status;
   mNumItems++;
   return 0;
}
