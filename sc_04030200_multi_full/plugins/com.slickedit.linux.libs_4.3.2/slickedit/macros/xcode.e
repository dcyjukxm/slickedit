...
_command void xcode_build_scheme(_str SchemeName='', _str WorkspaceFile=_workspace_filename) name_info(','VSARG2_REQUIRES_PROJECT_SUPPORT)
{
   ...
   _str associatedWkspace='';
   if(_IsWorkspaceAssociated(WorkspaceFile, associatedWkspace) && _IsXcodeWorkspaceFilename(associatedWkspace)) 
   {
       _str commandLine = 'xcodebuild -workspace ':+ maybe_quote_filename(WorkspaceFile) :+ ' -scheme ' :+ maybe_quote_filename(SchemeName);
...
void initMenuXcodeWorkspaceSchemes(int menu_handle)
{
    ...
    if (_IsWorkspaceAssociated(_workspace_filename, associateWkspace) && _IsXcodeWorkspaceFilename(associateWkspace)) {
