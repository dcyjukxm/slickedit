   ...
   if (mapid!='') {
      if (maptype=='SYSTEM') {
         ...
         if (status) {
            ...
            if (!was_mapped) {
               _str alertMsg = nls("Error processing DTD for file '%s'.\n\nFile '%s' not found",DocumentName,local_dtd_filename);
               _ActivateAlert(ALERT_GRP_WARNING_ALERTS, ALERT_DTD_LOAD_ERROR, alertMsg);
               return(1);
            }
         }
      } else if (maptype=='SCHEMA') {
         status=_mapxml_find_system_file(mapid,buf_name,local_dtd_filename,-1,was_mapped);
         if (status) {
            status=FILE_NOT_FOUND_RC;
            if (!was_mapped) {
               _str alertMsg = (nls("Error processing schema for file '%s'.\n\nFile '%s' not found",DocumentName,local_dtd_filename));
...
{
   //say("_xml_addTagsToColorCoding: tagfile="tagfile" prefix="((prefix!=null)? prefix:"(null)"));
   int status=tag_read_db(tagfile);
   //say('status='status);
   if (status < 0) {
      return;
   }
...
static void _mapxml_init_file2(boolean doUpdate,_str (&NamespaceHashtab):[],_str xsl_doctype_system='')
{
   ...
   //say('get status='status);
   //say('maptype='maptype' mapid='mapid' markid='markid);
   if (status) {
      ...
      if (xsl_doctype_system!='') {
         ...
         typeless el;
         for (el._makeempty();;) {
             NamespaceHashtab._nextel(el);
             if (el._isempty()) break;
             mapid=NamespaceHashtab:[el];
             maptype='SCHEMA';
             mapprefix=el;
             if (mapprefix!='xsi') {
                break;
...
void _xml_get_current_namespaces(_str (&NamespaceHashtab):[])
{
   ...
   //say('status='status);
   //if (status) say('status='get_message(status));
   ...
   // Could check if document not is XSL stylesheet tag but I think is over kill.
   _str prefix="";
   _str *pprefix=NamespaceHashtab2._indexin('http://www.w3.org/1999/XSL/Transform');
   if(pprefix) {
      prefix=*pprefix;
      int output_index=_xmlcfg_find_simple(handle,'/'prefix:+':stylesheet/'prefix':output');
      _str doctype_system='';
      if (output_index>=0) {
      ...
      if (doctype_system!='') {
         //say(doctype_system);
         //NamespaceHashtab:['']=doctype_system;
      } else if(!NamespaceHashtab2._indexin("http://www.w3.org/1999/xhtml")) {
         NamespaceHashtab:['']='http://www.w3.org/1999/xhtml';
      }
   }
   if (NamespaceHashtab._nextel(null)==null) {
   ...
   for (i._makeempty();;) {
       NamespaceHashtab._nextel(i);
       if (i._isempty()) break;
       say("index="i" value="NamespaceHashtab:[i]);
   }
#endif
}
...
void _ctl_add.lbutton_up ()
{
   ...
   _uri_items._lbsort();
   _uri_items._lbtop();
   _uri_items._lbsearch(newURIScheme);
   _uri_items.p_text = _uri_items._lbget_text();
   _uri_items._lbselect_line();
