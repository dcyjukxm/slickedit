...
int _save_non_active(_str &buf_name,boolean quit_option=false,int save_flags=SV_RETURNSTATUS)
{
   ...
   if ( !quit_option ||
      ...
      if (result==IDYES) {
         ...
         boolean is_dir = (buf_name != ''&& (isdirectory(buf_name) != ''));
         if (is_noname || is_dir) {
            fname := '';
            if (is_noname) {
            parse buf_name with '<' buf_id'>';
 ...
 *
 */
_command int list_modified(_str title='',boolean quiet=false, boolean hideCurrent=false) name_info(','VSARG2_REQUIRES_MDI)
{
   p_window_id=_mdi.p_child;
   _macro_delete_line();
   gstart_buf_id=p_buf_id;
   ...
   int width=0;
   _build_buf_list(width,p_buf_id,1,gstart_buf_id,hideCurrent);
