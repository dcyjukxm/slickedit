...
{
   curPath := _file_path(p_buf_name);
   if ( curPath!="" ) {
      if ( watchingSVN() ) {
         workspacePath := _file_path(_workspace_filename);
         STRARRAY curPathList;
         curPathList[0] = curPath;
         _SVNGetUpdatePathList(curPathList,workspacePath,auto pathsToUpdate);
         _str altWatchedPath = "";
         if ( pathsToUpdate[0]!=null ) {
            altWatchedPath = pathsToUpdate[0];
         }
         filewatcherAddPath(p_buf_name,WATCHEDPATH_SVN,false,altWatchedPath);
      }
   }
   return 0;
}
