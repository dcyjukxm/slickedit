...
int paste2(_str persistent_select='',int temp_view_id=0,_str name='',boolean isClipboard=true)
{
   ...
   if (!UseNamedClipboard &&
      ...
      int cbid=_cvtsysclipboard2(1,'CHAR','',p_UTF8,isClipboard,p_lexer_name);
      int view_id=0;
      get_window_id(view_id);
      ...
      down(Noflines-1);_end_line();
      boolean last_is_blank=!_first_non_blank_col(0);
      activate_window(view_id);
      // wholesaled from below for internal cbs
      if (p_TruncateLength > 0 && (Noflines == 1 || (Noflines == 2 && last_is_blank))
          && sel_lines <= 1) {
         int orig_col = p_col;
         boolean is_blank=!_first_non_blank_col(0);
         if (!is_blank) {
            _save_pos2(auto p2);
            _begin_line();
            ...
            _str leading_space = substr('',1,p_col-1);
            temp_line = leading_space :+ temp_line;
         }
         int new_bytes = length(temp_line);
   ...
   if ( _select_type(mark)=='BLOCK' && def_modal_paste && ! _insert_state() ) {
      ...
      if (p_TruncateLength > 0 && ((mark_name=='CHAR') && (Noflines == 1))
          && sel_lines <= 1) {
         // this code only supports single line pastes, and pasting over single line selections
         // an active selection being overwritten has been cleared at this point
         int orig_col = p_col;
         boolean is_blank=!_first_non_blank_col(0);
         if (!is_blank) {
            _save_pos2(auto p2);
            ...
            int begin_offset = (int)_QROffset();
            _TruncEndLine();
            // check for cursor past line end
            if (p_col < orig_col) {
               p_col = orig_col;
            }
            int end_offset = (int)_QROffset();
            ...
            // add how many spaces we need to factor in if we are on a blank line
            _str leading_space = substr('',1,p_col-1);
            temp_line = leading_space :+ temp_line;
         }
         int new_bytes = length(temp_line);
         // check if the truncation length will change with the paste
         int new_trunc_length = _TruncateLengthC(temp_line,new_bytes);
         if (new_trunc_length != 0 && new_bytes > new_trunc_length) {
            // restore the deleted selection if necessary
            if (sel_lines == 1) {
               undo();
            }
            message(get_message(VSRC_THIS_OPERATION_WOULD_CREATE_LINE_TOO_LONG));
...
_command old_list_clipboards,list_clipboards_modal() name_info(','VSARG2_MARK|VSARG2_TEXT_BOX|VSARG2_REQUIRES_EDITORCTL)
{
   ...
   if (!status) {
      ...
      _macro('m',_macro('s'));
      _macro_call('list_clipboards',name);
      activate_window(orig_wid);
   }
