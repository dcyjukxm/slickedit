...
#define VSCFGFILE_ALIASES "alias.slk"
 ...
 * @categories Configuration_Variables
 */
_str def_pl1_include_extensions;
...
const VSOPTION_MAXIMIZE_FIRST_MDICHILD          = 5;
...
/**
 * Symbol filters used by symbol tool window.
...
#if __MACOSX__
/**
 * If true, hide the MDI child window title bar when maximized
 */
boolean def_mac_hide_mdichild_title;
#endif
...
extern int _QtPrintDialogRestore(SEQtPrintOptions &options);
...
#define XSD_SETUP 'MN=XSD,TABS=+4,MA=1 74 1,KEYTAB=xml-keys,WW=1,IWT=0,ST=0,IN=2,WC=A-Za-z0-9_:$?.!\-,LN=XMLSCHEMA,CF=1,'
#define XMLDOC_SETUP 'MN=XMLDOC,TABS=+4,MA=1 74 1,KEYTAB=xml-keys,WW=1,IWT=0,ST=0,IN=2,WC=A-Za-z0-9_:$?.!\-,LN=XML,CF=1,'
#define VPJ_SETUP 'MN=VPJ,TABS=+4,MA=1 74 1,KEYTAB=xml-keys,WW=1,IWT=1,ST=0,IN=2,WC=A-Za-z0-9_:$?.!\-,LN=XML,CF=1,'
#define DOCBOOK_SETUP 'MN=DocBook,TABS=+4,MA=1 74 1,KEYTAB=xml-keys,WW=1,IWT=0,ST=0,IN=2,WC=\p{isXMLNameChar}?!,LN=XML,CF=1,'
#define XHTML_SETUP 'MN=XHTML,TABS=+4,MA=1 74 1,KEYTAB=xml-keys,WW=1,IWT=0,ST=0,IN=2,WC=\p{isXMLNameChar}?!,LN=XHTML,CF=1,'
#define ANT_SETUP 'MN=Ant,TABS=+4,MA=1 74 1,KEYTAB=xml-keys,WW=1,IWT=0,ST=0,IN=2,WC=A-Za-z0-9_:$?.!\-,LN=XML,CF=1,'
...
extern int _userLogFile(typeless &);
 ...
 */
extern int _CreateZipFile(_str filename, _str (&files)[], int (&zipStatus)[]);
...
boolean def_file_tabs_closable = false;
...
struct SavedStreamMarkers {
   int first_hotspot_index;
   int num_hotspots;
   int surround_index;
   int autobracket_index;
   int num_autobrackets;
   _str autobracket_closes[];   // Close_char for each of the saved autobracket pairs.
   int autobracket_keymasks[];
};
...
#define AUTOBRACE_PLACE_SAMELINE        0x0
#define AUTOBRACE_PLACE_NEXTLINE        0x1
#define AUTOBRACE_PLACE_AFTERBLANK      0x2
