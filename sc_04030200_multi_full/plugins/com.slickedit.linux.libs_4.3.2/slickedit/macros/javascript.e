...
defload()
{
   _str setup_info='MN=JavaScript,TABS=+4,MA=1 74 1,':+
                   'KEYTAB=c-keys,WW=1,IWT=0,ST=0,IN=2,WC=A-Za-z0-9_$,LN=JavaScript,CF=1,';
   _str compile_info='';
...
int _js_find_context_tags(_str (&errorArgs)[],_str prefixexp,
                          ...
                          VS_TAG_RETURN_TYPE (&visited):[]=null, int depth=0)
{
   //say("_js_find_context_tags("prefixexp","lastid","lastid_prefix")");
   ...
   if (prefixexp == '') {
      return _do_default_find_context_tags(errorArgs,prefixexp,lastid,lastidstart_offset,info_flags,otherinfo,find_parents,max_matches,exact_match,case_sensitive,filter_flags,context_flags,visited,depth);
   }
