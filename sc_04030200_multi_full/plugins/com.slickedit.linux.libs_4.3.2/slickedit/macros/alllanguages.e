...
void _language_form_apply(typeless pfnApplyControlCallback)
{
   ...
   if (langId == ALL_LANGUAGES_ID) {
      ...
      update_all_current_profiles(updateTabs, updateBraces, updateMisc);
   } else {
                                       ...
                                       AllLanguagesSetting (&alsTable):[], _str exclusions = '')
{
   ...
   if (updateKey != '') {
      ...
      maybeSetHtmlSchemeValue(updateKey, updateValue);
   }
}
...
void neutralizeAllLanguageControl(int wid, boolean saveRadioSetting)
{
   if (wid.p_visible && wid.p_name != '') {
      ...
      case OI_COMBO_BOX: 
         ...
         if (wid._lbsearch(LANGS_DONT_MATCH_CB)) {
            wid._lbtop();wid._lbup();
            wid._lbadd_item(LANGS_DONT_MATCH_CB);
         }
         wid.p_text = LANGS_DONT_MATCH_CB;
         set = true;
         break;
      case OI_TEXT_BOX: 
         // set it to the languages not matching
...
static void maybeSetHtmlSchemeValue(_str updateKey, typeless updateValue)
{
   switch (updateKey) {
   case TAG_CASING_UPDATE_KEY:
      set_html_scheme_value('tagcase', updateValue);
      break;
   case ATTRIBUTE_CASING_UPDATE_KEY:
      set_html_scheme_value('attribcase', updateValue);
      break;
   case VALUE_CASING_UPDATE_KEY:
      set_html_scheme_value('wordvalcase', updateValue);
      break;
   case HEX_VALUE_CASING_UPDATE_KEY:
      set_html_scheme_value('hexvalcase', updateValue);
      break;
   }
}
