 ...
 */
void updateAdaptiveFormattingSettings(int flag, boolean confirm = true)
{
   ...
   if (getNotificationMethod(NF_ADAPTIVE_FORMATTING) == NL_DIALOG && p_mdi_child && confirm) {
      ...
      */
      ...
      // brace style
      if (flag & AFF_BEGIN_END_STYLE) setBeginEndStyle(&scanner, changedFlags);
   
      // indent settings
      if (flag & (AFF_SYNTAX_INDENT | AFF_INDENT_WITH_TABS | AFF_TABS)) setIndentSettings(&scanner, flag, changedFlags);
   
      // paren settings
      if (flag & (AFF_NO_SPACE_BEFORE_PAREN | AFF_PAD_PARENS)) setParenSettings(&scanner, flag, changedFlags);
   
      // indent case from switch
      if (flag & AFF_INDENT_CASE) setIndentCaseFromSwitchSettings(&scanner, changedFlags);
   
      // keyword casing
      if (flag & AFF_KEYWORD_CASING) setKeywordCaseSettings(&scanner, changedFlags);
   
      // tag casing
      if (flag & AFF_TAG_CASING) setTagCaseSettings(&scanner, changedFlags);
   
      // attribute casing
      if (flag & AFF_ATTRIBUTE_CASING) setAttributeCaseSettings(&scanner, changedFlags);
   
      // value casing
      if (flag & AFF_VALUE_CASING) setValueCaseSettings(&scanner, changedFlags);
   
      // hex value casing
      if (flag & AFF_HEX_VALUE_CASING) setHexValueCaseSettings(&scanner, changedFlags);
