      ...
      if( !buffer_input ) {
         for( i=first_arg; i <= arg() && arg(i)!=''; i++ ) {
            ...
            if( checkbox ) {
               ...
               if( (temp_label_wid._text_width(tlabel)+255) > checkbox_width ) {
                  checkbox_width=temp_label_wid._text_width(tlabel)+255;
               }
            } else {
               parse current_line with tlabel':'init_value;
               tlabel=tlabel:+':';
 ...
 * @categories Forms
 */
int comboBoxDialog(_str formTitle, _str label, _str (&items)[], int flags = 0, _str defaultItem = '', _str help = '')
{
   return show('-modal _combobox_form', formTitle, label, items, flags, defaultItem, help);
}

_ctl_ok.on_create(_str formTitle, _str label, _str (&items)[], int flags = 0, _str defaultItem = '', _str help = '')
{
   ...
   if (defaultItem == '' || _ctl_combo._lbsearch(defaultItem)) {
      _ctl_combo._lbtop();
   }
   _ctl_combo._lbselect_line();
   _ctl_combo.p_text = _ctl_combo._lbget_text();
