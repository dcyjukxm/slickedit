...
class SearchResults {
   ...
   void insertMessage(_str line) {
      ...
      _lineflags(NEXTLEVEL_LF, LEVEL_LF | CURLINEBITMAP_LF);
      activate_window(orig_view_id);
   }
...
int _mffindGoTo(_str filename,int linenum,int col,boolean maybePushBM=false)
{
   ...
   if (_isno_name(filename)) {
      ...
      // check if untitled buffer still exists
      get_window_id(auto orig_view_id);
      activate_window(VSWID_HIDDEN);
      _safe_hidden_window();
      ...
      if (buf_id!='') {
         ...
         if (isEclipsePlugin()) {
            // special case for eclipse when a search result is contained in an open non-se editor
            int wid = _eclipse_open(0,filename'::'linenum);
            if (wid <= 0) {
            ...
            boolean isa_fso = (file_already_loaded || file_exists(filename));
            boolean isa_dir = (!file_already_loaded && isa_fso && isdirectory(filename));
            if (!isa_fso || isa_dir) {
               message(get_message(FILE_NOT_FOUND_RC));
               return FILE_NOT_FOUND_RC;
            }
