...
class NavMarker {
   ...
   void set(long offset, int side = 0, _str msg = '')
   {
      m_side = side;
      if (m_marker < 0) {
         m_marker = _StreamMarkerAdd(p_window_id, offset, 1, true, 0, getMarkerType(m_side), (def_display_nav_hints == 0) ? '' : msg);
         ...
         _StreamMarkerSetMessage(m_marker, (def_display_nav_hints == 0) ? '' : msg);
