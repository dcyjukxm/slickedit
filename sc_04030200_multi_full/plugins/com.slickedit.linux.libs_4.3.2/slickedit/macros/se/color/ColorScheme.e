...
class ColorScheme : IColorCollection, sc.lang.IEquals {
   ...
   void loadDefaultColorScheme() {
      ...
      m_embedded[CFG_TRUNCATION_COL_LINE] = truncationColumnLineColor;
       ...
       // modified file tabs color
      ColorInfo modifiedFileTabColor(0xFF, 0xFFFFFF, 0);
      m_colors[CFG_MODIFIED_FILE_TAB] = modifiedFileTabColor;
      m_embedded[CFG_MODIFIED_FILE_TAB] = modifiedFileTabColor;
   ...
   int saveColorScheme() {
      ...
      for ( cfg:=1; cfg<=CFG_LAST_DEFAULT_COLOR; ++cfg) {
         ...
         insert_line(cfgName:+"fg=":+dec2hex(m_colors[cfg].getForegroundColor(&this)));
         insert_line(cfgName:+"bg=":+dec2hex(m_colors[cfg].getBackgroundColor(&this)));
         ...
         if (cfg < m_embedded._length() && isEmbeddedColor(cfg) && m_embedded[cfg] != null) {
            insert_line(cfgName:+"em=":+dec2hex(getEmbeddedBackgroundColor(cfg)));
         }
      }
