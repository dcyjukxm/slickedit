...
class ExtensionSettings {
   ...
   private static typeless getExtensionDefVar(_str ext, _str defVarKey, _str defaultValue)
   {
      ...
      index_flags := (_files_case_sensitive()) ? MISC_TYPE : MISC_TYPE|IGNORECASE_TYPE;

      // find our guy in the names table
      index := find_index(defVarName, index_flags);
   ...
   private static int setExtensionDefVar(_str ext, _str defVarKey, typeless value)
   {
      ...
      if (index) {
         ...
         if (!isValueDefault) {
            index = insert_name(defVarName, MISC_TYPE, value);
