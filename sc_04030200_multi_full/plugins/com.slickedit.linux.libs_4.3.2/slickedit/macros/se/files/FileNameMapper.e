...
class FileNameMapper {
   ...
   public void getSimpleMaps(_str (&pathMap):[], FILELANGMAP_FILEPATTERN (&patternMap)[])
   {
      // make a simplified list of the patterns - the C++ code doesn't 
      // need all the data
      buildSimpleMap(patternMap);
      pathMap = m_files;
   }

   private void buildSimpleMap(FILELANGMAP_FILEPATTERN (&simpleList)[])
   {
      // make a simplified list of the patterns - the C++ code doesn't 
      // need all the data
      for (i := 0; i < m_patterns._length(); i++) {
         ...
         if (m_patterns[i].Regex == '' || m_patterns[i].Regex == null) {
