   ...
   public void buildOptionsHistory(int changeFlags, _str (&history)[])
   {
      ...
      if (historyIndex > 0) {
         ...
         for (i = 0; i < history._length(); i++) {
            ...
            method := (int)_xmlcfg_get_attribute(m_xmlConfigHandle, index, "ChangeMethod", OCM_CONFIGURATION);
            methodStr := configurationMethodToString(method);
            
            // compile our list
            history[i] = caption\tpath\ttime\tmethodStr;
         }
      }
