...
//         ctl_wksp_filter.forceFilterList(true);
      ...
      if (FileListGetInfo(auto path,auto LineNumber)) {
...
_command void tbfilelist_menu_command(_str cmdline="") name_info(',')
{
   ...
   case "prefixmatch":
      ...
      filterWid := FileListGetCurrentFilterWid();
...
void ctl_file_list."C-C","C-ins"()
{
   ...
   curFilename := _TreeGetCaption( treeCurIndex );
 ...
 */
void ctl_file_list.enter,"A-E","M-E"()
{
   open_selected_files();
}
 ...
 */
void ctl_file_list.on_create(FILELIST_FLAGS flags=0,FILELIST_SHOW_STATE filelist_show=FILELIST_SHOW_LAST)
{
   ...
   _str list_to_show = "";
   if ( filelist_show== FILELIST_SHOW_OPEN_FILES ) {
   ...
   if (p_name == list_to_show) {
      _SetDialogInfoHt("filelist",null,ctl_file_list);
      ...
      _TreeGetColButtonInfo(1,auto widthCol1,auto buttonFlagsCol1,auto stateCol1,auto captionCol1);

      if ( p_name=="ctl_file_list" ) {
         if ( buttonFlagsCol1&TREE_BUTTON_PUSHED ) {
...
void ctl_file_list.on_change(int reason,int index)
{
   ...
   if (reason==CHANGE_SELECTED) {
      ...
      if (_GetTagwinWID() && _get_focus()==p_window_id) {
...
static void open_selected_files()
{
   ...
   for (;;ff=0) {
      ...
      if (index <= 0) break;
      status = cur_tree_wid.FileListEditFile(index,"",&editCommands);
   }
   _TreeDeselectAll();
...
void ctl_close_button.lbutton_up()
{
   ...
   int (*pbuffer_id_hash):[] = GetPointerToFileListHashTab();
...
static int FileListGetCurrentFilterWid(int tabNumber=-1)
{
   ...
   if (!formwid) return 0;
   if ( tabNumber < 0 ) {
      tabNumber = formwid.ctl_sstab.p_ActiveTab;
   }
   switch ( tabNumber ) {
   case FILELIST_SHOW_OPEN_FILES:
      return formwid.ctl_filter.p_window_id;
   case FILELIST_SHOW_PROJECT_FILES:
      return formwid.ctl_proj_filter.p_window_id;
   case FILELIST_SHOW_WORKSPACE_FILES:
      return formwid.ctl_wksp_filter.p_window_id;
   }
   return 0;
}
...
static int FileListGetCurrentTreeWid(int tabNumber=-1)
{
   ...
   switch ( tabNumber ) {
   case FILELIST_SHOW_OPEN_FILES:
      return formwid.ctl_file_list.p_window_id;
   case FILELIST_SHOW_PROJECT_FILES:
      return formwid.ctl_project_list.p_window_id;
   case FILELIST_SHOW_WORKSPACE_FILES:
      return formwid.ctl_workspace_list.p_window_id;
   }
   return 0;
}
...
static int FileListGetWorkspaceTreeWid()
{
   formwid := FileListGetFormWid();
   if (!formwid) return 0;
   return formwid.ctl_workspace_list.p_window_id;
...
static int FileListGetProjectTreeWid()
{
   formwid := FileListGetFormWid();
   if (!formwid) return 0;
   return formwid.ctl_project_list.p_window_id;
...
{
   int formwid=FileListGetFormWid();
   if ( formwid <= 0 ) return;
   _SetDialogInfoHt("filelist",newValue,formwid.ctl_file_list);
}

static typeless *GetPointerToFileListHashTab()
{
   int formwid=FileListGetFormWid();
   if ( formwid <= 0 ) return null;
   return _GetDialogInfoHtPtr("filelist",formwid.ctl_file_list);
}
...
static int FileListAddBuffer(int tree_wid, boolean isCurrent)
{
   ...
   if (index > -1) {
      ...
      pHashtab := GetPointerToFileListHashTab();
...
static void FileListUpdateOpenFiles()
{
   ...
   cur_buf_id := _mdi.p_child.p_buf_id;
...
{
   _str filter=FileListGetCurrentFilterWid().p_text;
#if __UNIX__
...
static int FileListGetInfo(_str &path, int &LineNumber, int index=-1)
{
   ...
   int bid=treewid._TreeGetUserInfo(index);
   if (bid == '-') {
      // category
      return 0;
...
{
   if ( forceUpdate || gfilelist_show==FILELIST_SHOW_OPEN_FILES) {
      formwid := FileListGetFormWid();
      ...
      if ( forceUpdate || ((time_now - time) < 0) || ((time_now-time) > 1000)) {
         file_list := FileListGetFileTreeWid();
         if (!file_list) return;
         // keep track of the current buffer ID
         cur_buf_id := _mdi.p_child.p_buf_id;
         if ( !forceUpdate && (_get_focus() == file_list) ) {
            ...
            if ( index>=0 ) {
               ...
               orig_state:=state;
               if ( _mdi.p_child.tbfilelist_is_modified() ) {            
                  nodeFlags|=TREENODE_FORCECOLOR;
                  state=1;
               }else{
                  nodeFlags&=~(TREENODE_ALTCOLOR|TREENODE_FORCECOLOR);
                  state=0;
               }
...
void _switchbuf_filelist(_str oldbuffname, _str flag)
{
   ...
   tree_wid := FileListGetFileTreeWid();
   if (!tree_wid) return;
   int (*pbuffer_id_hash):[]=GetPointerToFileListHashTab();
...
static void restoreButton()
{
   ...
   if ( formwid ) {
      int wid=p_window_id;
      p_window_id=formwid;
      if ( gfilelist_show==FILELIST_SHOW_OPEN_FILES ) {
         showOpenFiles();
