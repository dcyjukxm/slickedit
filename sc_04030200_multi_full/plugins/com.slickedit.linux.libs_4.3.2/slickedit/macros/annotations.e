                                       ...
                                       _str new_bufname, int buf_flags)
{
   ...
   if (_filter && _filter_key) {
      ...
      if (!_filter._lbsearch(filter)) {
         _filter.p_text = filter;
         if (!_filter_key._lbsearch(filterKey)) {
            _filter_key.p_text = filterKey;
         }
      }
   }
   ...
   while ((bofReached == false) || (eofReached == false)) {
      ...
      if (!(totalScores._indexin(curLine))) {
         ...
         if (lineScore >= CANDIDATE_MATCH) {
            ++windowsChecked;
            windowScore = getWindowScore(lm, curLine, compare);
