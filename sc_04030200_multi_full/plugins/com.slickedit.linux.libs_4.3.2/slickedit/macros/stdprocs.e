...
*/
   ...
   if (findAliases && aliasfilename!='') {
      ...
      boolean inmem=1;
      typeless status=load_files('+q +m +b 'aliasfilename);
      if (status) {
         inmem=0;
         status=load_files(_load_option_encoding(aliasfilename)' +q +m 'maybe_quote_filename(aliasfilename));
      ...
      if (!status) {
         top();_begin_line();
         while (!search('^'_escape_re_chars(word),'@rh'LangCaseSensitiveStr)) {
            if (alias_info=='') alias_info=p_line;
            //curword=cur_word(junk);//Fix this later
            _str line='';
            get_line(line);
            int p=pos('( |$|\()',line,1,'r');
            _str curword=substr(_rawText(line),1,p-1);
...
int _update_picture(int index,_str filename)
{
   ...
   if (index<0) {
      if (index==FILE_NOT_FOUND_RC) {
         ...
         _message_box(nls('Unabled to load picture %s1',filename)'. 'get_message(index));
      }
   } else {
