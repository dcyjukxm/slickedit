      ...
      if (_in_comment(false)) {
         ...
         if (((_GetCommentEditingFlags(VS_COMMENT_EDITING_FLAG_SPLIT_LINE_COMMENTS) && _will_split_insert_line()) || (_GetCommentEditingFlags(VS_COMMENT_EDITING_FLAG_EXTEND_LINE_COMMENTS) && at_end_of_line())) &&
            ...
            if (_GetCommentEditingFlags(VS_COMMENT_EDITING_FLAG_AUTO_XMLDOC_COMMENT) &&
               ...
               (_GetCommentEditingFlags(VS_COMMENT_EDITING_FLAG_AUTO_DOC_COMMENT) && c_maybe_create_xmldoc_comment(double_slash_bang ? '//!' : '///')) ) {
               CW_doccomment_nag();
               return;
            }
            //Try to handle with comment wrap.  If comment wrap
 ...
 */
boolean c_maybe_create_xmldoc_comment(_str slcomment_start = '///')
{
...
_command void c_slash() name_info(','VSARG2_CMDLINE|VSARG2_REQUIRES_EDITORCTL)
{
   ...
   if (_GetCommentEditingFlags(VS_COMMENT_EDITING_FLAG_AUTO_DOC_COMMENT) && c_maybe_create_xmldoc_comment()) {
      CW_doccomment_nag();
      return;
   } else {
      keyin('/');
...
void _c_endbrace(boolean inhibit_beautify=false) {
   ...
   if ( command_state() || cfg==CFG_STRING || p_window_state:=='I' ||
      ...
      if (find_matching_paren(true) == 0) {
         ...
         new_beautify_range(start, endo, markers, true, true, false, BEAUT_FLAG_TYPING);
      } else {
         restore_pos(sp);
         next_char();
...
static int c_expand_space()
{
   ...
   if ( word=='main' ) {
      ...
      expansion_end = maybe_insert_braces(noSpaceBeforeParen, insertBraceImmediately,width,word,c_else_followed_by_brace_else(word));
      add_hotspot();
   } else if ( word=='else') {
      typeless p;
      typeless s1,s2,s3,s4;
   ...
   if (expansion_end >= expansion_start
      ...
      new_beautify_range(expansion_start, expansion_end, markers, true);
