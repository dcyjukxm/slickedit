 ...
 * 
 */
void vi_cur_word_boundaries2(int &startCol, int &endCol, int leading_spaces = 0, int trailing_spaces = 0, _str dir = "+")
{
   cur_char := get_text();
 ...
 * 
 */
void vi_cur_word_boundaries(int &startCol, int &endCol, int leading_spaces = 0, int trailing_spaces = 0, _str dir = "+")
{
 ...
 */
_command int vi_scroll_window_up(typeless count=1) name_info(','VSARG2_CMDLINE|VSARG2_REQUIRES_EDITORCTL|VSARG2_LASTKEY|VSARG2_READ_ONLY)
{
   _str key="";
   if( command_state() ) {
      key=last_event();
 ...
 */
_command int vi_scroll_window_down(typeless count=1) name_info(','VSARG2_CMDLINE|VSARG2_REQUIRES_EDITORCTL|VSARG2_LASTKEY|VSARG2_READ_ONLY)
{
   _str key="";
   if( command_state() ) {
      key=last_event();
...
_command int vi_delete(typeless count="", _str cb_name="") name_info(','VSARG2_MARK|VSARG2_CMDLINE|VSARG2_REQUIRES_EDITORCTL|VSARG2_LASTKEY)
{
   ...
   if( vi_name_eq_translate(vi_name_on_key(key1)):==name ) {
      ...
      */
      ...
      case '{':
      case 'B':
         vi_find_enclosing_bracket('{', '-', repeat_count);
         break;
      case '(':
      case 'b':
         vi_find_enclosing_bracket('(', '-', repeat_count);
...
static int vi_change(_str option, typeless count=1, _str cb_name="")
{
   ...
   if( option:=='C' ) {
      ...
      int startCol, endCol;
      vi_cur_word_boundaries(startCol, endCol);
...
_command int vi_change_line_or_to_cursor(typeless count="", _str cb_name="") name_info(','VSARG2_CMDLINE|VSARG2_REQUIRES_EDITORCTL|VSARG2_LASTKEY)
{
   ...
   if( vi_name_on_key(key1):==vi_name_on_key(key2) ) {
      ...
      _str key3=vi_get_event();
      if (vi_name_on_key(key3):=='vi-next-word') {
         status=vi_change('W',count,cb_name);
      } else {
         vi_message('Invalid key sequence');
         return(1);
      }
 ...
 */
_command void vi_toggle_char_visual() name_info(','VSARG2_CMDLINE|VSARG2_LASTKEY|VSARG2_MARK|VSARG2_REQUIRES_EDITORCTL|
VSARG2_READ_ONLY)
{
 ...
 */
_command void vi_toggle_line_visual() name_info(','VSARG2_CMDLINE|VSARG2_LASTKEY|VSARG2_MARK|VSARG2_REQUIRES_EDITORCTL
|VSARG2_READ_ONLY)
{
   if (!vi_visual_maybe_command()) {
      vi_switch_mode('V');
 ...
 */
_command void vi_toggle_block_visual() name_info(','VSARG2_CMDLINE|VSARG2_LASTKEY|VSARG2_MARK|VSARG2_REQUIRES_EDITORCTL|
VSARG2_READ_ONLY)
{
   if (!vi_visual_maybe_command()) {
      vi_switch_mode('V');         
