...
void _ftpopenFillProfiles(boolean set_textbox)
{
   ...
   if( p_Noflines ) {
      ...
      if( set_textbox ) {
         ...
         // selected to change.
         typeless status=_lbsearch(profile_name,'i');
         _lbselect_line();
      }
   }
...
static void oncreateFTPOpen()
{
   ...
   typeless status=_ctl_profile._lbsearch(profile);
   if( !status ) {
...
void _ftpopenFillCwdHistory(boolean set_textbox)
{
   ...
   if( p_Noflines ) {
      if( set_textbox ) {
         ...
         // from the list, but we do not want the current item
         // selected to change.
         typeless status = _lbsearch(cwd,_ftpFileCase(fcp_p));
         _lbselect_line();
      }
   }
   _ftpopenChangeRemoteCwdOnOff(1);
