...
void _InitializeFileNameMapping()
{
   if (!def_file_name_map_init_c_value) {
      def_file_name_mapper.init();
...
void _ctl_ok.on_create(int type, _str fileOrPattern = '', _str language = '')
{
   ...
   if (language != '' && !_ctl_language_combo._lbsearch(language)) {
      _ctl_language_combo._lbselect_line();
      _ctl_language_combo.p_text = _ctl_language_combo._lbget_text();
   }
}
