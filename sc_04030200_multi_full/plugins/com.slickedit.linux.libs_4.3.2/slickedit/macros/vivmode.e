 ...
 * @return 
 */
_command int vi_visual_yank() name_info(','VSARG2_MARK|VSARG2_CMDLINE|VSARG2_REQUIRES_EDITORCTL|VSARG2_LASTKEY|VSARG2_READ_ONLY){
   if (!vi_visual_maybe_command()) {
      begin_select();
      vi_cut(1,VI_CB0);
      vi_visual_toggle();
 ...
 * @return
 */
_command int vi_visual_put() name_info(','VSARG2_MARK|VSARG2_CMDLINE|VSARG2_REQUIRES_EDITORCTL|VSARG2_LASTKEY){
   int repeat_count,i;
   if (!vi_visual_maybe_command()) {
      vi_repeat_info(last_index(),last_event(),arg(1),arg(2));
      repeat_count=vi_repeat_info('C',arg(1));
 ...
 * @return 
 */
_command int vi_visual_toggle() name_info(','VSARG2_MARK|VSARG2_CMDLINE|VSARG2_REQUIRES_EDITORCTL|VSARG2_LASTKEY|VSARG2_READ_ONLY){
   if (!vi_visual_maybe_command()) {
      if (in_char_visual_mode) {
         vi_toggle_char_visual();
      } else if (in_line_visual_mode) {
         vi_toggle_line_visual();
      } else if (in_block_visual_mode) {
         vi_toggle_block_visual();
      }
      return(0);
...
_command int vi_visual_i_cmd(int count = 1) name_info(','VSARG2_MARK|VSARG2_CMDLINE|VSARG2_REQUIRES_EDITORCTL|VSARG2_LASTKEY|VSARG2_READ_ONLY) {
   ...
   case 'w':
      vi_visual_select_inner_word(0,0,0,count,dir);
      break;
   case 'W':
      vi_visual_select_inner_word(1,0,0,count,dir);
      break;
   case 's':
      vi_visual_select_inner_sentence(0,0,count,dir);
      break;
   case 'p':
 ...
 * @see vi_visual_i_cmd  
 */
static void vi_visual_select_inner_sentence(int leading_spaces = 0, int trailing_spaces = 0, int count = 1, _str dir = "+"){
   int i;
   for (i = 0; i < count; i++) {
 ...
 * @see vi_visual_i_cmd  
 */
static void vi_visual_select_inner_paragraph(int leading_spaces = 0, int trailing_spaces = 0, int count = 1, _str dir = "+"){
   int i;
   for (i = 0; i < count; i++) {
      size := vi_visual_get_num_chars() + 1;
      if (size > 1) {
 ...
 * @see vi_visual_i_cmd  
 */
static void vi_visual_select_inner_word(int word_type = 0, int leading_spaces = 0, int trailing_spaces = 0, int count = 1, _str dir = "+"){
   int i;
   for (i = 0; i < count; i++) {
      size := vi_visual_get_num_chars() + 1;
      if (size > 1) {
      ...
      if (word_type == 0) {
         vi_cur_word_boundaries(sc, ec, leading_spaces, trailing_spaces, dir);
      } else if (word_type == 1) {
         vi_cur_word_boundaries2(sc, ec, leading_spaces, trailing_spaces, dir);
      }
 ...
 */
void vi_visual_finish_i_cmd(typeless &p,typeless &p1){
   deselect();
   save_pos(p);
