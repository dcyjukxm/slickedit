...
int _GitPull(_str filename)
{
   ...
   _str cmd = 'pull '_GitGetNameForRemoteRepository(repositoryRoot)' '_GitGetBranch(repositoryRoot);
   _str output[];
   _str shellOption = '';
   if (def_git_pushpull_interactive == false) {
      shellOption = 'Q';
...
int _GitCommit(_str filelist[],_str comment)
{
   ...
   for (i = 0; i < filelist._length(); i++) {
      ...
      boolean ismodified = _CVSBufferIsModified(filename);
      if ( ismodified ) {
...
static int _GitShell(_str command,_str path,_str shell_options,_str (&output)[], boolean suppressOutput=false)
{
   ...
   if ((status != 0) || (def_git_output_all == true)) {
      ...
      WriteToOutputWindow('------------- 'dateTime.toStringParts(DT_LOCALTIME, DT_DATE)' 'dateTime.toStringParts(DT_LOCALTIME, DT_TIME));
      WriteToOutputWindow('Command: 'commandWithoutOutput);
      // maybe write the output results
      if (suppressOutput == false) {
         WriteArrayToOutputWindow(output);
      }
      WriteToOutputWindow('');
   }
 ...
 */
static void WriteToOutputWindow(_str output)
{
   _str temp[];
   temp[0] = output;
   WriteArrayToOutputWindow(temp);
}

/**
 * Writes an array of strings to the output window
 */
static void WriteArrayToOutputWindow(_str output[])
{
