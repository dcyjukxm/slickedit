      ...
      if ( filename != '' ) {
         ...
         if (!status) {
            ...
            if (messageCount < def_build_messages_limit) {
               //Try to find errors and warnings. Everything else is counted as an
               //'(Info)'
               msgType = siftBuildErrors(err_msg);

               se.messages.Message tmpMsg;
               tmpMsg.m_creator = BUILD_MESSAGE_TYPE;
               tmpMsg.m_type = msgType;
...
static _str siftBuildErrors (_str& errMsg)
{
   if (pos(def_build_errors_re, errMsg, 1, 'R') > 0) {
      return 'Error';
   }
   if (pos(def_build_warnings_re, errMsg, 1, 'R') > 0) {
      return 'Warning';
   }
