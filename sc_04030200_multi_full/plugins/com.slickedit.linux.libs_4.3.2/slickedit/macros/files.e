...
_command e,edit(_str filenameArg='', typeless a2_flags='',  _str auto_create_firstw_arg='') name_info(FILE_MAYBE_LIST_BINARIES_ARG'*,'VSARG2_CMDLINE|VSARG2_REQUIRES_MDI|VSARG2_NOEXIT_SCROLL)
{
   ...
   */
   ...
   if (!_no_child_windows() && !(a2_flags&EDIT_NOSETFOCUS)) {
      ...
      if (final_wid!=VSWID_HIDDEN  &&
          NextWindow_wid!=VSWID_HIDDEN &&
...
*/
static _str set_first_window(_str &filename,_str auto_create_firstw_arg)
{
...
//   endif
      ...
      if ( substr(word_nq,1,1)=='+' || substr(word_nq,1,1)=='-' ) {
         ...
         if ( letter=='T' || letter=='V' || letter=='B' ) {
            ...
            if (!ExplicitWindowSizeGiven) {
               ...
               first_window=set_first_window('+t',auto_create_firstw_arg);
            }
      ...
      if ( substr(word_nq,1,1)=='@' ) {   /* List of files? *///ch1
         ...
         for (;;) {
            ...
            if ( status ) {
               // suppress the 'too many windows' error here in eclipse
               if ( status!=NEW_FILE_RC && (status!=TOO_MANY_WINDOWS_RC && isEclipsePlugin()) ) { 
                       ...
                       boolean forcePromptingIfModified=true)
{
   ...
   if ((def_actapp&ACTAPP_SUPPRESSPROMPTUNLESSMODIFIED) || !allowPrompting) {
      ...
      if (( def_actapp & ACTAPP_WARNONLYIFBUFFERMODIFIED)) {
         ...
         if (useWholeFileCompare() && DSBackupVersionExists(doc_name)) {
            int file_wid = 0;
            int backup_wid = 0;
            int orig_wid = 0;
            int status1 = _open_temp_view(doc_name, file_wid, orig_wid, "+d");
            int status2;
            int version = DSGetNumVersions(doc_name) - 1;
            backup_wid = DSExtractVersion(doc_name, version, status2);
            if (!status1 && !status2) {
               int different = FastCompare(file_wid, 0, backup_wid, 0);
               if (different) {
                  result = show('-modal _auto_file_reload_form', doc_name);
               } else {
                  result = IDNO;
                  ...
                  orig_wid.p_file_date = (long)_file_date(orig_wid.p_buf_name,'B');
               }
            }
            _delete_temp_view(file_wid);
            _delete_temp_view(backup_wid);
         } else {
            result = show ('-modal _auto_file_reload_form', doc_name);
         }
      }
...
#endif
      _str reloadResult = select_tree(reloadBufNames, reloadBufIDs, reloadBitmaps,
                                  ...
                                  int reloadThreshold=0)
{
   ...
   for( i=0; i<bufIds._length(); ++i ) {
      ...
      if (p_buf_name != '' && isdirectory(p_buf_name)) {
         p_modify = true;
         bufferIDs[bufferNumber] = p_buf_id;
         delFileNames:[bufferNumber] = p_buf_name;
         numModifiedOrDeleted++;

      } else if (bfiledate == "") { //If there is no file on disk ...
         if ((p_file_date != "" && p_file_date != 0) && 
             !(p_buf_flags & VSBUFFLAG_PROMPT_REPLACE) && 
             _HaveValidOuputFileName(p_buf_name) && //but we're editing one (so not creating a new file).
             (p_modify == false)) { 
         ...
         if (useWholeFileCompare() && DSBackupVersionExists(p_buf_name)
             && ((int)_time('b')-(int)origTime)<def_autoreload_timeout_threshold
             ) {
            // 6/10/2010 - Compare the current file contents with the last backup 
            // history entry.  We have to compare with the backup history archive
            // because if the file is modified in the buffer, and the date has 
            // changed on disk, we cannot tell if the file on disk changed or not.
            int orig_pBufId = p_buf_id;
            int file_wid = 0;
            int backup_wid = 0;
            int orig_wid2 = 0;
            int status1 = _open_temp_view(p_buf_name, file_wid, orig_wid2, "+d");
            int status2;

            backup_wid = DSExtractMostRecentVersion(orig_wid2.p_buf_name, status2);
            if (!status1 && !status2) {
               int different = FastCompare(file_wid, 0, backup_wid, 0);
               if (!different) {
                  ...
                  // If we do not do this, the user will be prompted when they
                  // save the file because the file on disk will still be newer
                  orig_wid2.p_file_date = (long)_file_date(orig_wid2.p_buf_name,'B');
               }
               _delete_temp_view(file_wid);
               _delete_temp_view(backup_wid);
