...
int _ctl_scheme_save.lbutton_up(_str doRename='')
{
   ...
   if ( do_rename ) {
      ...
      status=_ctl_schemes_list._lbsearch(old_name,'I');
      if ( !status ) {
         _ctl_schemes_list._lbdelete_item();
   ...
   if ( _ctl_schemes_list._lbsearch(name) ) {   // Is it already in the list?
      _ctl_schemes_list._lbadd_item(name);
...
_ctl_scheme_delete.lbutton_up()
{
   ...
   typeless status=_ctl_schemes_list._lbsearch(old_name,'I');
   if ( !status ) {
      _ctl_schemes_list._lbdelete_item();
      _ctl_schemes_list._lbtop();
   }
...
#define DEFAULT_BEAUTIFIER_EXT  "phpscript=c ada=ada c=c java=c js=c cs=c e=c html=html cfml=html xml=html xsd=html vpj=xml docbook=xml ant=xml as=c d=c"   /* Valid beautifier languages */
...
_command void beautifier_options() name_info(','VSARG2_REQUIRES_EDITORCTL|VSARG2_MARK)
{
   if (new_beautifier_supported_language(p_LangId)) {
