...
static boolean isEndVerb(_str & word)
{
   ...
   if (pos("END-", upcase(word)) != 1) return(false);
...
static void skipWHENClause(boolean & reachedEOF)
{
   ...
   while (!down()) {
      ...
      if (upcase(word) == "WHEN" || upcase(word) == "END-EVALUATE") {
         up();
         return;
      }
                              ...
                              )
{
   ...
   if (upcase(imperativeKeyword) == "WHEN") {
   ...
   while (true) {
      ...
      if (upcase(word) == upcase(imperativeKeyword)) {
         if (firstCondition) {
                                ...
                                )
{
   ...
   */
   ...
   while (keyMaybeOnSameLine || !down()) {
      ...
      if (isEndVerb(word) || (!keyMaybeOnSameLine && isVerb(word) && !foundImperativeKey)) {
         up();
         endLine = lastNonEmptyLine;
         return;
      }
      ...
      if (!foundImperativeKey) {
         ...
         // statement.
         if (upcase(verb) != "EVALUATE" && upcase(word) == "WHEN") {
            up();
            endLine = p_line;
            return;
         } else if (upcase(verb) != "IF" && upcase(word) == "ELSE") {
            up();
            endLine = p_line;
            return;
         } else if (upcase(verb) != "IF" && upcase(word) == "END-IF") {
            up();
            endLine = p_line;
            return;
         }
...
static boolean isSpecialCasePERFORM()
{
   ...
   cobNextWord(word, reachedEOF, true);
