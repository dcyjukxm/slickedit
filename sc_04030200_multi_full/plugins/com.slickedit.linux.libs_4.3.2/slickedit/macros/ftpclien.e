...
#if __VERSION__ >= 17.0 && __VERSION__ <= 18.0102
   // 9/10/2013 - rb
   // v17 - v18.0.1 : Workaround for bug where pasting text into text field
   // of PSCBO_EDIT combox calls ON_CHANGE() with reason=CHANGE_CLINE_NOTVIS 
   // instead of  CHANGE_CLINE_OTHER. Note that this workaround disables
   // support for using Up/Down/ScrollWheel events to select from list (a
   // reasonable tradeoff in my opinion).
   if ( reason == CHANGE_CLINE_NOTVIS ) {
      return;
   }
#endif
...
void _tbFTPClient_form.on_resize()
{
   ...
   if (isEclipsePlugin()) {
      int ftpClientContainer = _ftpclientQFormWid();
      if(!ftpClientContainer) return;
      old_wid = p_window_id;
      p_window_id = ftpClientContainer;
      eclipse_resizeContainer(ftpClientContainer);
   }
