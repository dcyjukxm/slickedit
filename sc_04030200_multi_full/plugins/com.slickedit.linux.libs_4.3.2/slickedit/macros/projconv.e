...
static NEWFOLDERINFO NewFolderInfo[]={
   {"Source Files", "*.c;*.C;*.cc;*.cpp;*.cp;*.cxx;*.c++;*.prg;*.pas;*.dpr;*.asm;*.s;*.bas;*.java;*.cs;*.sc;*.e;*.cob;*.html;*.rc;*.tcl;*.py;*.pl;*.d;*.m;*.mm"},
...
#if __UNIX__
   "source files"   => "*.c;*.C;*.cc;*.cpp;*.cp;*.cxx;*.c++;*.C++;*.prg;*.pas;*.dpr;*.asm;*.s;*.bas;*.java;*.sc;*.e;*.cob;*.html;*.rc;*.tcl;*.py;*.pl;*.d"
   ,"header files"  => "*.h;*.H;*.hh;*.hpp;*.hxx;*.inc;*.sh;*.cpy;*.if"
   ,"resource files"=> "*.ico;*.cur;*.dlg"
   ,"bitmaps"       => "*.bmp;*.xpm;*.xbm"
   ,"other files"   => ALLFILES_RE
#else
   "source files"   => "*.c;*.cc;*.cpp;*.cp;*.cxx;*.c++;*.prg;*.pas;*.dpr;*.asm;*.bas;*.java;*.cs;*.sc;*.e;*.cob;*.html;*.rc;*.tcl;*.py;*.pl;*.vb;*.d"
   ...
   for (i=0;i<DependencyProjects._length();++i) {
      DependencyProjects[i]=
         _RelativeToWorkspace(
            _AbsoluteToProject(
               translate(DependencyProjects[i],FILESEP,FILESEP2),
...
{
   _str retVal = '';
   _str queryString = item;
   // try to find the value in the normal project file
   int groupIndex = _xmlcfg_find_simple(projectHandle, "/Project/PropertyGroup[not(@Label)]");
   if (groupIndex >= 0) {
      if (config != '') {
         queryString = item"[@Condition=\"'$(Configuration)|$(Platform)'=='"config"'\"]";
      }
      int index = _xmlcfg_find_simple(projectHandle, queryString, groupIndex);
      if (index >= 0) {
         int data = _xmlcfg_get_first_child(projectHandle, index, VSXMLCFG_NODE_PCDATA);
         if (data >= 0) {
            retVal = _xmlcfg_get_value(projectHandle, data);
