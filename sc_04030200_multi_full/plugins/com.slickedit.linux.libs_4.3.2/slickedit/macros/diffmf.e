   ...
   case DIFF_CAPTION2:
      {
         ...
         if (!tree1._IsFunctionIndex(curindex1)) {
            ...
            _str SourceDiffOption=!(def_diff_options&DIFF_NO_SOURCE_DIFF)?' -sourcediff ':'';
            _mdi.p_child.diff(iniRestoreOption' -RegisterAsMFDChild 'fid' -nomapping  -RefreshTagsClose 'curindex1' 'curindex2' 'AutoCloseOption' 'SourceDiffOption' 'maybe_quote_filename(Path1:+File1)' 'maybe_quote_filename(Path2:+File2));
...
static int DelKey()
{
   ...
   for (;;) {
      ...
      if (_TreeIsSelected(index1)) {
         ...
         --NumSelected;
         if (!NumSelected) break;
      }
      if (!gotnext) {
         index1=_TreeGetNextIndex(index1);
      }
   }
   p_window_id=origwid;
   return(0);
