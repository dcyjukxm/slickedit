...
static int _GetAllFilePathsForProject(_str projectFilename,_str workspaceFilename,STRARRAY &pathList)
{
   ...
   status := _getProjectFiles(workpacePath, absolute(projectFilename,workpacePath), fileList, 1);
...
_command int svn_update(_str filename='') name_info(FILE_ARG'*,')
{
   ...
   ismodified := _CVSBufferIsModified(filename);
   if ( ismodified ) {
 ...
 */
static boolean pathIsParentDirectory(_str path,_str parentPath)
{
   lenPath := length(path);
...
static void getTopSVNPath(_str curPath,_str topPath,_str &topSVNPath)
{
   lastCurPath := curPath;
   for ( ;; ) {
      if ( !pathIsParentDirectory(curPath,topPath) ) {
