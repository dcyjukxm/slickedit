                     ...
                     boolean ReturnHashTab=false,boolean DisableTree=false)
{
   ...
   _lbsort();
   _lbtop();
   status := _lbsearch(GetProjectDisplayName(_strip_filename(Project,'P')),_fpos_case);
...
static void initialize_file_tab()
{
   ...
   if (_workspace_filename!='') {
      ...
      status=_lbsearch(_strip_filename(_project_name,'PE'));
      if (!status) {
         ctladd_to_project_name.p_text = _lbget_text();
      }
      p_window_id=wid;
   }
...
_command int workspace_retag(boolean quiet = false) name_info(',')
{
   ...
   if (!quiet) {
      ...
      rebuildAll = _param1;
      tagOccurrences = _param3;
      useThread = useThread && (_param5 != 0);
   }

   // if the tag file rebuild fails, tell them why
   status = _workspace_update_files_retag(rebuildAll, true, useThread, false, tagOccurrences, true, useThread);
   if (status < 0) {
