...
static int oncreateGeneral()
{
   ...
   _ctl_server_type._lbsearch(type_name);
   _ctl_server_type._lbselect_line();
   _ctl_server_type.p_text=_ctl_server_type._lbget_seltext();
   ...
   ctl_auth_type._lbsearch(type_name);
   ctl_auth_type._lbselect_line();
   ctl_auth_type.p_text=ctl_auth_type._lbget_seltext();
   ...
   _ctl_host_type._lbsearch(type_name);
   _ctl_host_type._lbselect_line();
   _ctl_host_type.p_text=_ctl_host_type._lbget_seltext();
...
void _ctl_list.rbutton_down()
{
   // p_scroll_left_edge>=0 indicates that we are in scroll mode
   int nochange_event;
...
_command void ftpProfileManagerRename() name_info(','VSARG2_NCW|VSARG2_READ_ONLY)
{
   ...
   if( old_profile!="" ) {
      ...
      if( status ) {
         ...
         listWid.set_scroll_pos(listWid.p_left_edge,0);
         listWid._lbselect_line();
      }
   }
