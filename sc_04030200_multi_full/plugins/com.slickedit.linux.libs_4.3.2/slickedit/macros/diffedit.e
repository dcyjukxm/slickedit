...
static void UpdateScrollThumbs(boolean updateImmediately=false)
{
   ...
   if (!_dont_use_diff_command) {
      ...
      if (DiffCommands._indexin(command_name)) {
         ...
         if (pos('\-space$',command_name,1,'r')) {
            ...
            _diff_docharkey();
         }else if (command_name=="embedded-key") {
            diff_maybe_embedded_key();
         }else{
...
static void diff_maybe_embedded_key()
{
   if (!_LanguageInheritsFrom("jcl")) {
      return;
   }
   int wid=0;
...
//   _CallbackBufSuspendAll(_ctlfile2.p_buf_id,0);
   ...
   if (DialogIsDiff() && (def_diff_edit_options&DIFFEDIT_AUTO_JUMP)) {
