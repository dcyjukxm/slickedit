                                       ...
                                       VS_TAG_RETURN_TYPE (&visited):[], int depth=0)
{
   ...
   if(index) {
      call_index(false, idexp_info, visited, depth, index);
   }
...
//   }
   ...
   if(beautify_new_function == "1") {
      ...
      new_beautify_range(start_off, _QROffset(), markers, true);
   }
...
_command void generate_debug() name_info(','VSARG2_REQUIRES_EDITORCTL)
{
   ...
   index := _FindLanguageCallbackIndex("_%s_get_expression_info");
   if(index) {
      call_index(false, idexp_info, visited, index);
