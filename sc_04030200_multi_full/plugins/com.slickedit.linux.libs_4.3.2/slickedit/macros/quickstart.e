...
void _quick_start_colors_form_restore_state()
{
   ...
   if (purpose == OP_QUICK_START) {
      ...
      if (currentScheme != ORIG_COLOR_SCHEME && currentScheme != _ctl_color_scheme_list.p_text) {
         ...
         if (!_ctl_color_scheme_list._lbsearch(currentScheme)) {
            _ctl_color_scheme_list.p_text = _ctl_color_scheme_list._lbget_text();
...
void fillInColorSchemes()
{
   ...
   if (_ctl_color_scheme_list._lbsearch(currentScheme)) {
      _ctl_color_scheme_list._lbsearch('Default');
   }
   _ctl_color_scheme_list.p_text = _ctl_color_scheme_list._lbget_text();
   _ctl_color_scheme_list._lbselect_line();
...
void _ctl_font_name_list_non_unicode.on_create()
{
   ...
   ctl_non_unicode_mode_name._lbsearch(_Ext2LangId("c"));
   ctl_non_unicode_mode_name.p_text = _LangId2Modename(_Ext2LangId("c"));
   ctl_unicode_mode_name._lbaddModeNames();
   ctl_unicode_mode_name._lbsort();
   ctl_unicode_mode_name._lbsearch(_Ext2LangId("html"));
   ctl_unicode_mode_name.p_text = _LangId2Modename(_Ext2LangId("html"));
...
static void fillInFonts(int fontNameList, _str fontName)
{
   ...
   // the list box.
   fontNameList._lbsearch(fontName);
   fontNameList.p_text = fontNameList._lbget_text();
...
void _quick_start_coding_form_apply()
{
   ...
   if (setIndent || setBraces) {
      ...
      update_all_current_profiles(setIndent, setBraces, false);
   }
...
void _ctl_welcome_label.on_create()
{
   ...
   _ctl_info_html._minihtml_ShrinkToFit();
   if (isEclipsePlugin()) {
      ctl_slickedit_pic.p_picture = _update_picture(-1, "vse_profile_eclipse_256.png");
