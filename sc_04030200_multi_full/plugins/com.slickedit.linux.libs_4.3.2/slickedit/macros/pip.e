...
static void _pip_log_send_result(_str result, _str date = gPipLastSendDate)
{
   ...
   DateTime then = DateTime.fromTimeB(gPipLastSendDate);
   date = then.toStringISO8601();
   msg := "Product Improvement Program result ("date") - ";
...
void _cb_participate.on_create()
{
   ...
   if (isEclipsePlugin()) {
      ctl_slickedit_pic.p_picture = _update_picture(-1, "vse_profile_eclipse_64.png");
   }
}
