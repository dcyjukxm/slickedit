...
static void _close_tile_or_window()
{
   typeless view_id=0;
   if (p_window_state=='N') {
      if ( _tile_above(view_id)!='') {
         activate_window(view_id);
         delete_tile('','','',DOWN);
         return;
      } else if (_tile_below(view_id)!=''){
         activate_window(view_id);
         delete_tile('','','',UP);
         return;
      } else if (_tile_left(view_id)!=''){
         activate_window(view_id);
         delete_tile('','','',RIGHT);
         return;
      } else if (_tile_right(view_id)!=''){
         activate_window(view_id);
         delete_tile('','','',LEFT);
         return;
      }
...
_command hsplit_window(_str load_options = "") name_info(FILE_ARG'*,'VSARG2_READ_ONLY|VSARG2_CMDLINE|VSARG2_MARK|VSARG2_REQUIRES_MDI_EDITORCTL|VSARG2_NOEXIT_SCROLL)
{
   ...
   one_tile();
   if ( p_window_state=='M' ) {   /* Window is maximized. */
      if (def_one_file!="") {
         p_tile_id=_create_tile_id();
      }
   ...
   _str old=x " "y " "width " "top_height;
   _no_resize=1;
   //_MDIChildSetWindow x,y,width,top_height
   //p_old_x=x;p_old_y=y;p_old_width=width;p_old_height=top_height;
   save_pos(auto p);
   int status=load_files("+i:"x" "(y+top_height)" "width" "(height-top_height)" n "filename);
   _no_resize='';
   if ( status ) {
   ...
   p_old_x=x;p_old_y=y+top_height;p_old_width=width;p_old_height=height-top_height;
   /* new=x " "(y+top_height) " "width " "(height-top_height) */
...
_command vsplit_window(_str load_options = "") name_info(FILE_ARG'*,'VSARG2_READ_ONLY|VSARG2_CMDLINE|VSARG2_MARK|VSARG2_REQUIRES_MDI_EDITORCTL|VSARG2_NOEXIT_SCROLL)
{
   ...
   _no_resize=1;
   save_pos(auto p);
   int status=load_files("+i:"(x+top_width)" "y" "(width-top_width)" "height" n "filename);
   _no_resize='';
   if ( status ) {
      return(status);
   }
   ...
   p_old_x=x+top_width;p_old_y=y;p_old_width=width-top_width;p_old_height=height;
   int next_wid=p_window_id;
   p_window_id=orig_wid;
   _no_resize=1;
   _MDIChildSetWindow(x,y,top_width,height);
   //_move_window(x,y,top_width,height);
   p_old_x=x;p_old_y=y;p_old_width=top_width;p_old_height=height;
   _no_resize='';
 ...
 * 
 */ 
_command void tile_windows(_str option="V") name_info(','VSARG2_READ_ONLY|VSARG2_ICON|VSARG2_REQUIRES_MDI_EDITORCTL|VSARG2_REQUIRES_TILED_WINDOWING|VSARG2_NOEXIT_SCROLL)
{
   if (isEclipsePlugin()) {
 ...
 * 
 */ 
_command shrink_window() name_info(','VSARG2_READ_ONLY|VSARG2_REQUIRES_MDI_EDITORCTL|VSARG2_REQUIRES_TILED_WINDOWING|VSARG2_NOEXIT_SCROLL)
{
   int view_id=0;
                            ...
                            typeless start_key)
{
   ...
   if ( start_key=='') {
      ...
      case 'A':  // Window above
         ...
         my=p_y;mx=child_x+child_width intdiv 2;
         break;
      case 'B':  // Window below
         my=p_child+p_height;mx=child_x+child_width intdiv 2;
         p_mouse_pointer=MP_SIZENS;
         break;
      case 'L':  // Left Window
      ...
      mou_set_xy(mx,my);
#ifdef not_finished
      _mdi._save_draw_setup(draw_setup);
      _mdi.p_fill_style=PSFS_TRANSPARENT;
      _mdi.p_draw_width=3;_mdi.p_draw_style=PSDS_INSIDE_SOLID;_mdi.p_draw_mode=PSDM_XORPEN;
...
#ifdef not_finished
      _mdi._draw_rect(rect_x1,rect_y1,rect_x2,rect_y2,color,0,'e');
#endif
...
#ifdef not_finished
        if (start_key=='') _mdi._draw_rect(rect_x1,rect_y1,rect_x2,rect_y2,color,0,'e');
#endif
        break;
...
static _str _tile_below(int &view_id)
{
   ...
   int edge_y2=edge_y1;
   view_id=(typeless) find_window_with_edge(edge_x1,edge_y1,edge_x2,edge_y2);
   return(view_id);
}
static _str _tile_above(int &view_id)
{
   /* IF there is not a window above. */
   int window_x, window_y, window_width, window_height;
    _MDIChildGetWindow(window_x,window_y,window_width,window_height);
...
static _str _tile_left(int &view_id)
{
   /* IF there is not a window above. */
   int window_x, window_y, window_width, window_height;
    _MDIChildGetWindow(window_x,window_y,window_width,window_height);
...
static _str _tile_right(int &view_id)
{
   /* IF there is not a window above. */
   int window_x, window_y, window_width, window_height;
    _MDIChildGetWindow(window_x,window_y,window_width,window_height);
 ...
 * (<b>p_tile_id</b>) as the current window is returned.  Otherwise, '' is 
 * returned.
 * 
 * @appliesTo  Edit_Window
 * @categories Window_Functions
 */
_str find_window_with_edge(int x1,int y1,int x2,int y2)
{
  int first_window_id=p_window_id;
  int tile_id=p_tile_id;
  /* for each window in the active ring of windows. */
 ...
 * @categories Window_Functions
 */
_str find_window_with_point(int x,int y)
{
   int first_window_id=p_window_id;
   int tile_id=p_tile_id;
   int view_id=0;
 ...
 * @categories Window_Functions
 */
int find_window_closest_to_point(int direction)
{
   int first_window_id = p_window_id;
   int tile_id = p_tile_id;
