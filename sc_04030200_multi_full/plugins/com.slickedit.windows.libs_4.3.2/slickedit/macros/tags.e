...
_str tags_filename(_str lang="",boolean includeSlickC=true)
{
   ...
   _str cppTagFile = '';
   // We want the C++ compiler tag file for "c", but "d" derives from "c"
   ...
   // sort of JDK tagging.
   if ((_LanguageInheritsFrom("c", lang) && !_LanguageInheritsFrom('d', lang)) ||
...
int tag_get_current_include_info(VS_TAG_BROWSE_INFO &cm) 
{
   ...
   typeless orig_values;
   _str orig_lang = p_LangId;
   embedded := _EmbeddedStart(orig_values);
   _str lang = p_LangId;
   int status=0;
   orig_line_no := p_line;
   if (_LanguageInheritsFrom('cob') || (embedded && _LanguageInheritsFrom('cob', orig_lang))) {
      ...
      if (!status && color==CFG_KEYWORD && match_length()) {
         ...
         word=strip(word, 'B',"'");
         _str filename='';
         filename=path_search(word,cobol_copy_path,"v",get_cobol_copy_extensions());
         if (filename=='') {
            if (pathlen(p_buf_name)) {
               filename=path_search(word,_strip_filename(p_buf_name,"n"),"v",get_cobol_copy_extensions());
            }
         }
         if (filename!='') {
   ...
   } else if (_LanguageInheritsFrom('pl1') || (embedded && _LanguageInheritsFrom('pl1', orig_lang))) {
      // first get the project include file path
      ...
      if (!status && (color==CFG_KEYWORD || color==CFG_PPKEYWORD) && match_length()) {
         ...
         filename=path_search(word,project_include_path,"v",get_pl1_include_extensions());
         if (filename!='') {
            cm.file_name=filename;
            if (embedded == 1) {
               _EmbeddedEnd(orig_values);
   ...
   if (context_id > 0 && _clex_find(0,'g')!=CFG_COMMENT) {
      ...
      if (type_name=='include' && p_line==include_line) {
         ...
         if (include_path != '' && _istagging_supported(p_LangId)) {
            ...
            // strip " (not found)"
            if (endsWith(include_path, " (not found)")) {
               parse include_path with include_path " (not found)";
            }
            cm.file_name=include_path;
            return(0);
         }
...
static _str tag_select_callback(int sl_event, typeless user_data, typeless info=null)
{
   ...
   case SL_ONINITFIRST:
      ...
      if ((last_command == 'push-tag' || 
         ...
         combo1_wid._lbsearch(value);
         combo1_wid._lbselect_line();
         combo1_wid.p_text = combo1_wid._lbget_text();
