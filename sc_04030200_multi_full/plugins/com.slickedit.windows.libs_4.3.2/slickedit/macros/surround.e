...
/**
 * find the absolute path to the specified alias file
 *
 * @param found set to true if the file exists
 * @param fname the file to look for
 *
 * @return the absolute path of the file (if it exists) or where it should be stored (if it does not)
 */
static _str find_alias_file(boolean&found,_str fname)
{
   _str temp=usercfg_path_search(fname);

   if (temp=='') {
      found=false;
      return(_ConfigPath():+fname);
   }
   found=true;
   return(temp);
}
...
/**
 * Opens the alias file for the specified extension into a temp view
 *
 * @param ext the extension to use
 * @param temp_view_id set to the id of the view that is created and holds
 *                     the alias file
 * @param orig_view_id set to the id of the active view when this function
 *                     is called
 * @return true if the temp view was created
 */
static boolean open_alias_file(_str ext,int&temp_view_id,int&orig_view_id)
{
   boolean ret_value=true;
   if (ext=="") return false;

   // is there a filename for this extension stored in a def-var
   filename := LanguageSettings.getAliasFilename(ext, '');
   if (filename != '') {
      parse filename with filename (PATHSEP) .;
   } else {
      filename = ext'.als';
      LanguageSettings.setAliasFilename(ext, filename);
   }

   boolean found;
   _str old_aliases=find_alias_file(found,filename);
   if (found) {
      int status=_open_temp_view(old_aliases,temp_view_id,orig_view_id);
      if (status) {
         _message_box('failed to open alias file "'filename'."');
         ret_value=false;
      }
   } else {
      // no aliases for this extension yet
      orig_view_id=_create_temp_view(temp_view_id);
      if (orig_view_id:=='') {
         ret_value=false;
      } else {
         p_buf_name=old_aliases;
      }
   }
   return ret_value;
}

static void find_aliases(int (&aliases):[])
{
   aliases._makeempty();
...
static void update_existing_aliases(_str new_aliases_file)
{
   int existing_aliases:[];
   int new_aliases:[];

   find_aliases(existing_aliases);

   int temp_wid;
   int orig_wid;
   int status=_open_temp_view(new_aliases_file,temp_wid,orig_wid);
   ...
   typeless index;
   for (index._makeempty(),new_aliases._nextel(index);!index._isempty();new_aliases._nextel(index)) {
      if (existing_aliases._indexin(index)) {
         top();
         up();
         status=search('^':+_escape_re_chars(index),'Rh@');
         if (!status) {
            ...
            _select_char(mark_id);
            status=_delete_selection(mark_id);
         }
      }
   }
   ...
   bottom();
   get(maybe_quote_filename(new_aliases_file), '', 'A');
}
...
{
   _str new_aliases=get_env('VSROOT'):+'sysconfig'FILESEP'aliases'FILESEP'$'surround_version:+lang'.als';
   ...
   int temp_view_id;
   int orig_view_id;
   if (!open_alias_file(lang,temp_view_id,orig_view_id)) {
      return;
   }

   update_existing_aliases(new_aliases);

   // always save the file in the user's configuration directory
   file := '"':+_ConfigPath():+_strip_filename(p_buf_name,'P'):+'"';
   _save_file(build_save_options(file):+' ':+file);
...
/**
 * Check if any of the automatic surround with template definitions are out of date
 * and update them as necessary
 */
void update_surround_definitions()
{
   int current_surround_version=0;
   int version_index=find_index('def-surround-version',MISC_TYPE);
   if (!version_index) {
      // first time try to create the def-var
      version_index=insert_name('def-surround-version',MISC_TYPE);
      if (!version_index) {
         // could not create the def-var???
         return;
      }
   } else {
      // has been updated at least once before, find out what version
      _str n_info=name_info(version_index);
      if (isnumber(n_info)) {
         current_surround_version=(int)n_info;
      }
   }

   if (current_surround_version>=SURROUND_VERSION) {
      return;
   }

   boolean lang_found:[];
   _str all_languages[];
   all_languages._makeempty();

   // somehow updating interferes with the language
   // search, so try to find all the languages before
   // updating anything
   lang_index := name_match('def-language-',1,MISC_TYPE);
   while (lang_index) {
      _str def_var_name=name_name(lang_index);
      all_languages[all_languages._length()] = substr(def_var_name,14);
      lang_index = name_match('def-language-',0,MISC_TYPE);
   }

   // walk all languages through all versions
   while (current_surround_version<SURROUND_VERSION) {
      ++current_surround_version;
      lang_found._makeempty();

      for (lang_index=0;lang_index<all_languages._length();++lang_index) {
         lang := all_languages[lang_index];
         if (!lang_found._indexin(lang)) {
            lang_found:[lang]=true;
            sur_update_language(current_surround_version,lang);
         }
      }
   }

   // save the version
   set_name_info(version_index,current_surround_version);
}
...
static int maybe_use_arg(sur_selection_parameters& sur_params,_str caption)
{
   ...
   _str als = alias_match(full_alias, 1);
   _str first_match = als;

   while ((als!='') && (ret_value==0)) {
      // exact match?
      if (als==full_alias) {
         handle_surround_selection(sur_params,als);
         ret_value=1;
      } else {
         ++num_matches;

         // check if there are any more matches
         als = alias_match(full_alias, 0);
      }
   }
   alias_match(full_alias, 2);

   // if there is only one match and it is not an exact match
   if ((ret_value==0)&&(num_matches==1)) {
      handle_surround_selection(sur_params,first_match);
      ret_value=1;
   }
...
typeless get_surround_with_templates(_str search_string='') 
{
   ...
   _str als = alias_match(search_string, 1);
   _str temps[]; 
   temps._makeempty();
   boolean hashtab:[]; //look for duplicates
   hashtab._makeempty();

   while (als != '') {
      // remove the prefix and put spaces in the name
      _str caption=translate(substr(als, prefix_length+1),' ', '_');
      if (hashtab._indexin(caption)==0 && caption!='') {
         hashtab:[caption]=true;
         temps[temps._length()] = caption;
      }

      als = alias_match(search_string, 0);
   }

   alias_match(search_string, 2);
   temps._sort('ID');
   return temps;
}
...
_command int surround_with(_str prefix='',_str markid='',boolean prompt=true) name_info(',' VSARG2_REQUIRES_EDITORCTL | VSARG2_MARK)
{
   ...
   typeless result=show('_sellist_form -xy -mdi -modal',
                        "Surround With...",
                        SL_VIEWID|SL_SELECTCLINE|SL_SIZABLE, // Indicate the next argument is a view_id
                        temp_view_id,
                        "OK,Customize...",
...
void save_surround_state_to(SavedStreamMarkers& state, long (&markers)[]) {
   if (gSurroundBlockMarkerId < 0) {
   ...
   _save_pos2(auto p);
   state.surround_index = markers._length();
   p_line = gSurroundMarkerStartLine;
   p_col = 1;
...
void restore_surround_state_from(SavedStreamMarkers& state, long (&markers)[]) {
   if (state.surround_index < 0 || state.surround_index >= markers._length()) {
      return;
   }

   _GoToROffset(markers[state.surround_index]);
   start := p_line;
   
   _GoToROffset(markers[state.surround_index+1]);
   nsl := p_line - start - 1;
   if (nsl < 0) {
      nsl = 0;
   }
   _GoToROffset(markers[state.surround_index+2]);
   endl := p_line;

   _GoToROffset(markers[state.surround_index+3]);
   endl_end := p_line;
 ...
 * @param indents    indent values for each line 
 */
static void indent_surround(_str direction, _str (&origLines):[], int (&indents):[])
{
   ...
   if (index) {
      call_index(direction, indents, index);
      return;
   }

   // use the generic indent/unindent code
   if (direction=='+') {
      _indent_line(false,indents);
   } else {
      unindent_line(indents);
   }
}
                             ...
                             _str (&origLines):[], int (&indents):[])
{
   ...
   for (i=0; i<num_lines; ++i) {
      ...
      if (doIndent) {
         indent_surround('-',origLines,indents);
      }
      if (p_TruncateLength > 0 && after_trunc != "") {
         // restore the text after the truncation column
         int orig_trunc_col = p_TruncateLength;
         p_TruncateLength = 0;
         int orig_pcol = p_col;
         p_col = trunc_length + 1; 
         _insert_text(after_trunc);
         p_col = orig_pcol;
         p_TruncateLength = orig_trunc_col;
      }
      if (end_lines > 0) {
         up(end_lines);
      }
                           ...
                           _str (&origLines):[], int (&indents):[])
{
   ...
   for (i=0; i<num_lines; ++i) {
      ...
      int trunc_col = p_TruncateLength;
...
// say("do_surround_mode_keys: end_line="end_line);
   ...
   // hashtable for tracking how far each line is indented
   // used when line truncation is on
   int indents:[];
   indents._makeempty();
   ...
   for (;;) {
      ...
      if ( !AutoCompleteActive() ) {
         ...
         case 'cursor-up':
            ...
            unsurround_lines(start_line, end_line, num_lines, p, origLines, indents);
            continue;
         case 'cursor-down':
            maybe_delete_blank_line(first_time, start_line, end_line);
            num_lines = get_num_surround_lines(1);
            surround_lines(start_line, end_line, num_lines, p, origLines, indents);
            continue;
         case 'page-up':
