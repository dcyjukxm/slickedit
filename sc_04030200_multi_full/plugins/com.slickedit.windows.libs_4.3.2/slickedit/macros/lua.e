...
defload()
{
   _str setup_info='MN='LUA_MODE_NAME',TABS=+4,MA=1 74 1,':+
                   'KEYTAB='LUA_LANGUAGE_ID'-keys,WW=1,IWT=0,ST=0,IN=2,WC='LUA_WORDCHARS',LN='LUA_LEXERNAME',CF=1,';
   _str compile_info='';
...
static _str _lua_expand_space()
{
   ...
   if (word == 'if') {
      replace_line(line:+'  then'); _end_line(); p_col -= 5;

   } else if (word == 'elseif') {
      width = _lua_get_statement_indent(word);
      replace_line(indent_string(width):+'elseif  then'); _end_line(); p_col -= 5;

   } else if (word == 'else') {
      width = _lua_get_statement_indent(word);
      replace_line(indent_string(width)'else');
      if (_lua_expand_end(word)) {
         insert_line(indent_string(width)'end'); _end_line(); add_hotspot();
         up();
         nosplit_insert_line();
      ...
      replace_line(line:+'  do');
      insert_line(indent_string(width)'end'); add_hotspot();
      ...
      restore_pos(p);
      if (status) {
         replace_line(line); add_hotspot();
         insert_line(indent_string(width)'until'); add_hotspot();
         up();
         nosplit_insert_line();
         p_col=width+syntax_indent+1;
      } else {
      ...
      replace_line(line:+'  do'); add_hotspot();
      insert_line(indent_string(width)'end'); add_hotspot();
