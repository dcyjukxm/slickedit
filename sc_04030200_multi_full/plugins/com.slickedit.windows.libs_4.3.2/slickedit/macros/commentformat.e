...
#define  CW_DEFAULTDOCCOMMENTALIASFILE 'doccomment.als'
...
void commentwrap_SetNewJavadocState() {
   _str hotSpotOffsets[];
   getHotSpotOffsets(hotSpotOffsets);
   ...
   setHotSpotOffsets(hotSpotOffsets);
}
...
static int CW_inBlockComment(typeless &startPos = 0, typeless &endPos = 0, boolean fromCW_getCommentLineContent = false) {
   ...
   if (!fromCW_getCommentLineContent) {
      ...
      if (strip(CW_getLineToColumn()) != '' || (OriginalLine == p_line && p_col < OriginalCol && OriginalCol < p_col + length(CW_commentTLCstr))) {
         restore_pos(OriginalPos);
         return (CW_NOTCOMMENT);
      }
      restore_pos(endPos);
 ...
 * @return  
 */
_str getCWaliasFile(_str lang = p_LangId) 
{
