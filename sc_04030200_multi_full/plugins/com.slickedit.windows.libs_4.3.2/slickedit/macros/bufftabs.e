...
static void set_file_tabs_closable(boolean value)
{
   def_file_tabs_closable = value;

   sstabwid := get_tab_control();
   if (sstabwid > 0) {
      sstabwid.p_ClosableTabs = def_file_tabs_closable;
      refresh_file_tabs();
   }
}
...
_command void set_file_tab_sort_order(int value = -1) name_info(','VSARG2_CMDLINE|VSARG2_EDITORCTL)
{
   ...
   // turn the abbreviated file names off if not in alphabetic mode
   if (def_file_tab_sort_order != FILETAB_ALPHABETICAL) {
      def_file_tabs_abbreviates_files = false;
   }
...
boolean file_tabs_closable(boolean value = null)
{
   if (value == null) {
      value = def_file_tabs_closable;
   } else {
...
boolean file_tabs_abbreviate_caption(boolean value = null)
{
   if (value == null) {
      ...
      if (def_file_tab_sort_order == FILETAB_ALPHABETICAL || value == false) {
...
static void addBufferToFileTabArrays(int bufId)
{
   ...
   if (index < 0) {
      if (def_file_tab_new_file_position == FILETAB_NEW_FILE_ON_RIGHT) {
...
void _cbquit_MaxFirstWindow(int bufId, _str bufName, _str docName, int bufFlags)
{
   // ignore this callback if they are closing a hidden window 
   if (bufFlags & VSBUFFLAG_HIDDEN) {
      return;
   }

   // no child windows, then get out of here
   if (_no_child_windows()) {
      return;
   }

   // the window being closed must be _mdi.p_child
   if (_mdi.p_child.p_buf_id != bufId) {
      return;
   }

   // make sure there is only one buffer remaining open
   bufferCount := 0;
   buf_info := buf_match("", 1, "V");
   loop {
      if (rc) break;
   ...
   // make sure there is only one MDI child window open
   windowCount := 0;
   window_info := window_match("", 1, "N");
   loop {
      if (rc) break;
      if (!window_info) break;
      if (++windowCount > 1) return;
      window_info = window_match("", 0, "N");
   }
   
   window_state := _mdi.p_child.p_window_state;
   maxFirstWindow := _default_option('F');

   // compare this window state to our current value of max first window - maybe change!
   if (window_state == 'N' && maxFirstWindow) {
      _default_option('F', false);
      _config_modify_flags(CFGMODIFY_OPTION);
   } else if (window_state == 'M' && !maxFirstWindow) {
      _default_option('F', true);
      _config_modify_flags(CFGMODIFY_OPTION);
   }
}
...
void ctlsstab1.on_create()
{
   ...
   ctlsstab1.p_ClosableTabs = (def_file_tabs_closable != 0);
#if __MACOSX__
   //ctlsstab1.p_DocumentMode = true;
   // Draw tabs differently on Mac
...
void ctlsstab1.rbutton_up()
{
   ...
   _menu_insert(menu_handle,itemPos++,MF_ENABLED|(def_file_tabs_closable? MF_CHECKED:MF_UNCHECKED),
...
{
   set_file_tabs_closable(!def_file_tabs_closable);
}
