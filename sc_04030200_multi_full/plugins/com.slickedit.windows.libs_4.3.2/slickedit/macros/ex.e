...
typeless __ex_set_listchars(typeless val='')
{
   ...
   if(val == ''){
      ...
      if(val == ''){
         ...
         val = 'eol:' :+ substr(special_chars,VSSPECIALCHAR_EOLCH1+1,1);
      }
      return(val);
   } else {
      if(pos(',', val)){
...
int ex_set_special_char(_str s, _str &new_specials){
   ...
   if (pos('eol:', s) == 1) {
      ...
      if(length(val) != 1) {
         ...
         new_specials = val :+ substr(new_specials, VSSPECIALCHAR_EOLCH1+2);
      }
   } else if (pos('tab:', s) == 1) {
      val = substr(s,5);
      if(length(val) != 2) {
...
//          p_col=_text_colc(p,'I');
      ...
      if( !in_global ) {
         ...
         status=search(old_search_string,search_flags'@',old_replace_string);
      }
      if ( !status ) {
