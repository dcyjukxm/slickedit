...
void _toolbars_prop_form_init_for_options(_str formNameOrTab = '')
{
   ...
   ctl_apply_button_sizing.p_visible = false;
...
boolean _toolbars_prop_form_is_modified(_str settings:[])
{
   ...
   if (ctlunified.p_value!=_tbGetLastUnifiedToolbarState()) return true;
...
boolean _toolbars_prop_form_apply()
{
   ...
   _tbSetLastUnifiedToolbarState(ctlunified.p_value);
...
void ctl_apply_button_sizing.lbutton_up() 
{ 
   int fid = p_active_form; 
   fid.okToolbarButtonOptions(); 
   fid._tbPropsShowControls( ctlcatlist.p_line - 1 ); 
} 
