...
_ul2_listbox.lbutton_down,"s-lbutton_down","c-lbutton-down"()
{
   ...
   case MS_EXTENDED:
      ...
      case name2event("c-lbutton-down"):
         ...
         p_cursor_y=mou_last_y();
         p_cursor_x=p_left_edge+p_windent_x;
         typeless line_selected=_lbisline_selected();
         if (line_selected) {
            _lbdeselect_line();
         } else {
            _lbselect_line();
...
{
	// see if we are messing with the case-sensitivity
	newCS := false;
	if (options != '') {
...
/**
 * Searches for an item in the list box.  If the item is not
 * found, we add it.
 *
 * @param item          tem to search for.
 ...
 *
 */
int _lbfind_or_add_item(_str item, _str options = '', int addOption = 0)
{
   if (_lbfind_and_select_item(item, options)) {
