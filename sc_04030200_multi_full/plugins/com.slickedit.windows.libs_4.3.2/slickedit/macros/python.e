...
defload()
{
   _str setup_info='MN='PY_MODE_NAME',TABS=+4,MA=1 74 1,':+
                   'KEYTAB='PY_MODE_NAME'-keys,WW=1,IWT=0,ST=0,';
   _str compile_info='';
...
static boolean py_isIdChar(_str ch) {
   word_chars := _clex_identifier_chars();
   ...
   for (;;) {
      ...
      if (pos(match,PYTHON_PAREN_CHARS)/* || match==','*/) {
         ...
         if (p_col!=1) {
            left();
            status=search('[^ \t]|^','-rh@');
            isIDChar=!status && py_isIdChar(get_text());
         }
      ...
      */
      ...
      if (!pos(' 'match' ',PYTHON_BLOCK_WORDS) || py_isIdChar(chLeft) || py_isIdChar(chRight)) {
#if PY_DEBUG_INDENT
