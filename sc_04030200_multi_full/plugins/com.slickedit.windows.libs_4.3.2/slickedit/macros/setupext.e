...
boolean _is_syntax_indent_supported(_str langId)
{
   ...
   return (find_index('def-options-'langId, MISC_TYPE) > 0);
}
...
void _EncodingFillComboList(_str encoding='',_str defaultSetting='Automatic',int SkipFlags=0)
{
   // Get the encoding list.
   OPENENCODINGTAB openEncodingTab[];
   _EncodingListInit(openEncodingTab);
...
void _ctl_add.lbutton_up()
{
   ...
   if (!_ctl_languages._lbsearch(newMode)) {
...
void _manage_extensions_form_restore_state()
{
   ...
   if (!_ctl_extensions._lbsearch(curExt)) {
...
void applyChangesToCurrentExtensionInfo()
{
   ...
   if (ext == ALL_EXTENSIONS) {
      ...
      if (_LangId2Modename(langId) != _ctl_languages.p_text && _ctl_languages.p_text != '(None)') {
...
void _ctl_new.on_create()
{
   ...
   if (_UTF8()) {
      _EncodingFillComboList('','Automatic',OEFLAG_REMOVE_FROM_OPEN);
   } else {
...
static void _manage_extensions_form_initial_alignment()
{
   ...
   sizeBrowseButtonToTextBox(ctlAlternateEditor.p_window_id, ctlFilterAppCmdButton.p_window_id, 0, rightAlign);
}
...
void _ctl_new.lbutton_up()
{
   ...
   _ctl_extensions._lbselect_line();
   _ctl_extensions.top();
   _ctl_extensions._lbsearch(_param1, 'I');
   _ctl_extensions._lbselect_line();
...
void _ctl_delete.lbutton_up()
{
   ...
   _ctl_extensions._lbsearch(extension, 'I');
   _ctl_extensions._lbdelete_item();
   _ctl_extensions._lbselect_line();
   _ctl_extensions.call_event(CHANGE_CLINE, _ctl_extensions, ON_CHANGE, 'W');
}
...
void _ctl_extensions.on_change(int reason)
{
   ...
   case CHANGE_SELECTED:
      ...
      _ctl_languages.p_enabled = _ctl_lang_setup.p_enabled = ctlAlternateEditor.p_enabled = 
         ctlUseFileAssociation.p_enabled = ctlFilterAppCmdButton.p_enabled = ctllabel9.p_enabled = 
      ...
      if (ext == ALL_EXTENSIONS) {
         ...
         _ctl_languages._lbtop();
         _ctl_languages._lbselect_line();
         if (_ctl_languages._lbsearch('(None)')) {
            _ctl_languages._lbtop();
            _ctl_languages._lbadd_item('(None)');
         }
         _ctl_languages.p_text = '(None)';
         ...
         if (allMatch) {
            ...
            if (ctlencoding._lbsearch(EXTENSIONS_DIFFER)) {
               ctlencoding._lbtop();
               ctlencoding._lbadd_item(EXTENSIONS_DIFFER);
            }
            ctlencoding.p_text = EXTENSIONS_DIFFER;
         }
      } else {

         if (!ctlencoding._lbsearch(EXTENSIONS_DIFFER)) {
            ctlencoding._lbdelete_item();
         }
         if (!_ctl_languages._lbsearch('(None)')) {
            _ctl_languages._lbdelete_item();
         }
         // just a regular language
         langId := _Ext2LangId(ext);
         language := _LangId2Modename(langId);

         if (!_ctl_languages._lbsearch(language)) {
            _ctl_languages._lbselect_line();
            _ctl_languages.p_text = _ctl_languages._lbget_text();
         } else {
            _ctl_languages._lbtop();
...
_ok.on_create()
{
   ...
   if (!ctl_referto._lbsearch('Plain Text')) {
      ctl_referto._lbselect_line();
      ctl_referto.p_text = ctl_referto._lbget_text();
   } 
}
...
void _language_general_form_init_for_options(_str langID)
{
   ...
   if (def_keys!='ispf-keys') {
      ...
      yShift := ctlDiffColFrame.p_y - ctlBoundsFrame.p_y;
      ctlDiffColFrame.p_y -= yShift;
...
{
   if (ctlDiffColOn.p_value) {
      ctlDiffStartLab.p_enabled=ctlDiffEndLab.p_enabled=ctlDiffStart.p_enabled=ctlDiffEnd.p_enabled=1;
   } else {
      ctlDiffStartLab.p_enabled=ctlDiffEndLab.p_enabled=ctlDiffStart.p_enabled=ctlDiffEnd.p_enabled=0;
   }
}

void ctlTruncOn.on_create()
{
   if (isEclipsePlugin()) {
      ctlTruncAuto.p_visible=false;
   }
}
...
_str _language_word_wrap_form_apply_control(_str controlName, _str langId, _str value)
{
   ...
   case 'text3':
      ...
      newMargins = value' 'rightMargin' 'value;
...
boolean _language_comments_form_is_lang_included(_str controlName, _str langId, boolean allLangsExclusion)
{
   ...
   case 'ctlframe16':
      ...
      included = (_LanguageInheritsFrom('c', langId) ||
...
void _language_auto_complete_form_init_for_options(_str langID)
{
   ...
   // The languages that support the new beautifier have some settings
   // that moved from formatting to auto-complate.
   new_beaut := new_beautifier_supported_language(langID);
   _ctl_blankline.p_visible = new_beaut;
...
_syntax_expansion.lbutton_up()
{
   ...
   _insert.p_enabled = _syntax_expansion.p_value != 0 && BRACES_SUPPORTED;
...
void _language_auto_bracket_form_init_for_options(_str langID)
{
   ...
   if (supports_advanced_bracket_cfg(langID) && enable_braces) {
   ...
   _cb_ab_brace_pad.p_visible = _cb_ab_brace_pad.p_enabled = false;
...
_str _language_view_form_apply_control(_str controlName, _str langId, _str value)
{
   ...
   case '_ctl_special_chars':
      ...
      else newValue = oldValue & ~SHOWSPECIALCHARS_ALL;
...
{
   _ctl_tabs.p_value = _ctl_spaces.p_value = _ctl_newline.p_value = _ctl_special_chars.p_value;
}
...
_ctl_tabs.lbutton_up()
{
   ...
   if (_ctl_tabs.p_value && _ctl_spaces.p_value && _ctl_newline.p_value) _ctl_special_chars.p_value = 1;
}
...
_str _language_formatting_form_get_value(_str controlName, _str langId)
{
   ...
   case '_style2':
      ...
      value = '_style'style;
      break;
   case '_insert_blank_line':
      value = (int)LanguageSettings.getInsertBlankLineBetweenBeginEnd(langId);
      break;
   case '_no_space':
...
boolean _language_formatting_form_is_lang_included(_str controlName, _str langId, boolean allLangsExclusion)
{
   ...
   case '_style2':
      ...
      break;
   case '_insert_blank_line':
      optionFlag = LOI_INSERT_BLANK_LINE_BETWEEN_BEGIN_END;
      break;
   case '_no_space':
...
_str _language_formatting_form_apply_control(_str controlName, _str langId, _str value)
{
   ...
   case '_style2':
      ...
      break;
   case '_insert_blank_line':
      LanguageSettings.setInsertBlankLineBetweenBeginEnd(langId, ((int)value != 0));
      break;
   case '_no_space':
