...
static void InitMiscDiffInfo(DIFF_MISC_INFO &misc,_str Type)
{
   ...
   if ( gDiffSetupData.Interleaved ) {
      ...
      */
      ...
      if ( gDiffSetupData.balanceBuffersFirst ) {
         ...
         // Even though we will replace the buffer, we have to do this so that 
         // multi-file diff picks up the right language when it checks to see if
         // files match at the end.  This is important because there could be 
         // begin/end columns.
         Wid2._SetEditorLanguage(Wid1.p_LangId);

         _SetDialogInfoHt("Buf2Name",Wid2.p_buf_name,diff_form_wid._ctlfile1);
                          ...
                          boolean &SoftWrap=false)
{
   ...
   if (status) {
      InMem=0;
      _str opts=build_load_options(FileName);
      _str nowindow=lowcase(def_one_file=='+w')?'-w':'';
...
static int FastCompareFiles(_str filename1,_str filename2,int ProgressFormWID)
{
   ...
   do {
      ...
      if (NCSStatus1==STRING_NOT_FOUND_RC &&
         ...
         cols := LanguageSettings.getDiffColumns(lang1);
         ...
         if ( !(def_diff_options&DIFF_NO_SOURCE_DIFF) || colOnOff1 || colOnOff2 ) {
...
_command int merge(_str arg1='') name_info(FILE_ARG'*,'VSARG2_EDITORCTL)
{
   ...
   int PicType;
   MergeFiles(BaseFileViewId,Rev1ViewId,Rev2ViewId,OutputViewId,
