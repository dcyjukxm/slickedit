...
const C_NOT_FUNCTION_WORDS = ' int long double float bool short signed unsigned char asm __asm __declspec __except catch do for if return sizeof typeid switch throw typedef using while with template const_cast static_cast dynamic_cast reinterpret_cast volatile ';
...
{
   if (_chdebug) isay(depth, "_c_get_expression_info: possible_op="PossibleOperator" @"_QROffset()" ["p_line","p_col"]");
   int status=0;
   ...
   if (cfg==CFG_COMMENT) {
      ...
      if (!_inJavadocSeeTag(tag)) {
         if (_inDocComment()) {
            ...
            return _doc_comment_get_expression_info(PossibleOperator, idexp_info, visited, depth);
         }
   ...
   if (PossibleOperator && !(slickc && get_text_safe(1,(int)point('s')-1)=='_')) {
      ...
      if (!done) {
         ...
         if (!foundOperator && _LanguageInheritsFrom('c') && pos(get_text_safe(), "=+-!%^&*[]<>/| ")) { 
            ...
            if (get_text_safe()=='' && pos(get_text_safe(1,_nrseek()+1), "=+-!%^&*[]<>/|") /*&& !at_end_of_line()*/) { 
               right();
            }
            ...
            _str c_operator = _expand_tabsc(p_col+1,operator_end_col-p_col);
            ...
            int before_status = -1;
            if (c_operator != "" && pos('['word_chars']',get_text_safe(),1,'r')) {
            ...
            if (before_status==0 && idexp_info.prefixexpstart_offset>=2) {
               ...
               if (pos(get_text_safe(), "*~!&")) {
                  ...
                  idexp_info.prefixexpstart_offset-=2;
               }
            }
   ...
   _str before_keywords = ' gcnew new goto throw using import class struct union interface namespace ';
   if ((iscpp || slickc) && (gtk==',' || (gtk==TK_ID && !pos(' 'gtkinfo' ',before_keywords)) || gtk==':' || gtk=='>')) {
      /*
...
_str c_prev_sym()
{
   ...
   if ((ch=='"' || ch=="'" ) && _clex_find(0,'g')==CFG_STRING) {
      ...
      if (!status && p_line==start_line) {
         gtk=TK_STRING;
         gtkinfo=_expand_tabsc(p_col,start_col-p_col);
      }
      return(gtk);
   }
   if (_LanguageInheritsFrom("pl") && ch=="'") {
      ...
      if (!status) {
         ...
         if (num_matches == 0) {
            ...
            if (idexp_info.lastid!='' || match_symbol!='') {
               ...
               if (num_matches == 0 && (_LanguageInheritsFrom('c') || _LanguageInheritsFrom('cs')) && !pos(VS_TAGSEPARATOR_package, rt.return_type)) {
