...
class OptionsXMLParser {
   ...
   public boolean initParser(int purpose, _str file = '')
   {
      ...
      if (file != '') {
         ...
         if (m_xmlHandle > 0) {
            ...
            if (m_languageBegin > 0) populateLanguageNodes();
            if (m_versionControlBegin > 0) populateVersionControlNodes(false);
   ...
   private boolean initOptionsDOM()
   {
      ...
      if (m_xmlHandle > 0) {
         ...
         // load up languages
         populateLanguageNodes();
         populateUserDefinedLanguages();

         // load up version control
         populateVersionControlNodes();
   ...
   public _str getOptionsXMLVersion()
   {
      ...
      return _xmlcfg_get_attribute(m_xmlHandle, index, 'SlickEditVersion');
   }
   ...
   private void generateSearchTags()
   {
      ...
      _xmlcfg_find_simple_array(m_xmlHandle, '//DialogTemplates//Dialog/@TagFunction', tagFunctions,
   ...
   /**
    * Populates the language nodes in the options XML file.  The
    * XML file is saved with the languages listed, but not filled
    * in with the language templates.
    *
    */
   private void populateLanguageNodes()
   {
      // find the default language elements
      defLang := _xmlcfg_find_simple(m_xmlHandle, '//DefaultLanguageInfo');
      int parents:[];

      sysHelp := '';
      if (defLang > 0) {
         sysHelp = _xmlcfg_get_attribute(m_xmlHandle, defLang, 'SystemHelp');
      }

      // get the list of all the nodes that need replacing
      _str a[];
      ss := '//@'LANGUAGE_ATTRIBUTE;
      _xmlcfg_find_simple_array(m_xmlHandle, ss, a, m_topNode);

      if (a != null) {
         int i;
         for (i = 0; i < a._length(); i++) {

            langNode := _xmlcfg_get_parent(m_xmlHandle, (int)a[i]);

            // set the caption to be the Language name by grabbing the id
            lang := _xmlcfg_get_attribute(m_xmlHandle, langNode, LANGUAGE_ATTRIBUTE);
            caption := '';
            if (lang != '') {
               caption = _LangId2Modename(lang);
               if (caption == '') {
                  caption = getCaption(langNode);
                  if (caption == '') caption = lang;
               }

               // save the parent for sorting later
               parent := _xmlcfg_get_parent(m_xmlHandle, langNode);
               if (parent != m_languageBegin) {
                  parents:[parent] = parent;
               }
            }

            _xmlcfg_add_attribute(m_xmlHandle, langNode, 'Caption', caption, VSXMLCFG_ADD_ATTR_AT_BEGINNING);

            // see if we need to insert a category template here
            if (defLang > 0 && _xmlcfg_get_attribute(m_xmlHandle, langNode, 'CategoryTemplate') == 'DefaultLanguageInfo') {

               // set the system help attribute if it is not already there
               if (_xmlcfg_get_attribute(m_xmlHandle, langNode, 'SystemHelp') == '') {
                  _xmlcfg_add_attribute(m_xmlHandle, langNode, 'SystemHelp', sysHelp);
               }

               // copy the default stuff into our new language node
               firstCopy := _xmlcfg_copy(m_xmlHandle, langNode, m_xmlHandle, defLang, VSXMLCFG_COPY_CHILDREN);

               // check and see if there are any extra nodes that are not in the template
               int templateCaptions:[];
               child := _xmlcfg_get_first_child(m_xmlHandle, langNode);
               while (child != firstCopy) {

                  // add in any stuff that is not part of the template
                  sib := firstCopy;
                  ...
                  if (sib > 0) {
                     ...
                     _xmlcfg_copy(m_xmlHandle, langNode, m_xmlHandle, child, VSXMLCFG_COPY_AS_CHILD);
                  }
      ...
      // now that we're done, go through all the parents and sort them by caption
      int parent;
      foreach (parent in parents) {
         // sorts by the KEEP at top attribute first, then the caption
         _xmlcfg_sort_on_attribute(m_xmlHandle, parent, 'KeepAtTop', 'D', null, 'Caption', 'I');
      }
   }
    ...
    *
    */
   private void populateUserDefinedLanguages()
   {
      userDef := -1;

      // find the default language template
      defLang := _xmlcfg_find_simple(m_xmlHandle, '//DefaultLanguageInfo');
      sysHelp := _xmlcfg_get_attribute(m_xmlHandle, defLang, 'SystemHelp');
      ...
      for (;;) {
        ...
        if (!_IsInstalledLanguage(langId)) {
           if (first_char(name_info(index))!='@') {
              ...
              addNewLanguageToXML(langId, userDef, defLang, sysHelp);
           }
        }
   ...
   /**
    * Populates the version control nodes in the options XML file.
    * The XML file is saved with the languages listed, but not
    * filled in with the language templates.
    *
    */
   private void populateVersionControlNodes(boolean createUserAdded = true)
   {
      // find the default vc elements
      defVC := _xmlcfg_find_simple(m_xmlHandle, '//DefaultVersionControlInfo');

      sysHelp := '';
      dlgHelp := '';
      if (defVC > 0) {
         sysHelp = _xmlcfg_get_attribute(m_xmlHandle, defVC, 'SystemHelp');
         dlgHelp = _xmlcfg_get_attribute(m_xmlHandle, defVC, 'DialogHelp');
      }

      // get the list of providers (their providerID and display names)
      _str providerList:[];
      VersionControlSettings.getCommandLineProviderIDList(providerList);

      // get the list of all the nodes that need replacing
      _str a[];
      _xmlcfg_find_simple_array(m_xmlHandle, '//@'VC_PROVIDER_ATTRIBUTE, a, m_topNode);

      int index;
      foreach (index in a) {
         index = _xmlcfg_get_parent(m_xmlHandle, index);
         vcp := _xmlcfg_get_attribute(m_xmlHandle, index, VC_PROVIDER_ATTRIBUTE);
         caption := vcp;
         if (providerList._indexin(vcp)) {
            caption = providerList:[vcp];

            // set this to null so we know we've already done it
            providerList:[vcp] = null;

            _xmlcfg_add_attribute(m_xmlHandle, index, 'Caption', caption, VSXMLCFG_ADD_ATTR_AT_BEGINNING);

            if (defVC > 0 && _xmlcfg_get_attribute(m_xmlHandle, index, 'CategoryTemplate') == 'DefaultVersionControlInfo') {
               // set the dialog help attribute if it is not already there
               if (_xmlcfg_get_attribute(m_xmlHandle, index, 'DialogHelp') == '') {
                  _xmlcfg_add_attribute(m_xmlHandle, index, 'DialogHelp', dlgHelp);
               }
               _xmlcfg_add_attribute(m_xmlHandle, index, 'SystemHelp', sysHelp);
               _xmlcfg_copy(m_xmlHandle, index, m_xmlHandle, defVC, VSXMLCFG_COPY_CHILDREN);
            }
         } else {
      ...
      if (createUserAdded) {
         // now go through the remaining providers and add them individually
         // go through each of these providers and see if they already have nodes, if not, create one
         _str provID, provName;
         foreach (provID => provName in providerList) {

            if (provName != null) {
               // look for the provider ID attribute
               index = _xmlcfg_add(m_xmlHandle, m_versionControlBegin, 'Category',
                                        VSXMLCFG_NODE_ELEMENT_START_END, VSXMLCFG_ADD_AS_CHILD);

               _xmlcfg_add_attribute(m_xmlHandle, index, VC_PROVIDER_ATTRIBUTE, provID, VSXMLCFG_ADD_ATTR_AT_BEGINNING);
               _xmlcfg_add_attribute(m_xmlHandle, index, 'Caption', provName, VSXMLCFG_ADD_ATTR_AT_BEGINNING);
               _xmlcfg_add_attribute(m_xmlHandle, index, 'SystemHelp', sysHelp);
               _xmlcfg_add_attribute(m_xmlHandle, index, 'DialogHelp', dlgHelp);
      ...
      // now that we're done, sort the providers by name
    ...
    */
   public void addNewVersionControlProviderToXML(_str vcName)
   {
      // find the default language elements
      defVC := _xmlcfg_find_simple(m_xmlHandle, '//DefaultVersionControlInfo');
      ...
      if (defVC > 0) {
         ...
         provID := VersionControlSettings.getProviderID(vcName);
         _xmlcfg_add_attribute(m_xmlHandle, vcIndex, VC_PROVIDER_ATTRIBUTE, provID);

         // set help attributes
         _xmlcfg_add_attribute(m_xmlHandle, vcIndex, 'SystemHelp', sysHelp);
         _xmlcfg_add_attribute(m_xmlHandle, vcIndex, 'DialogHelp', '');

         // copy the default stuff into our new language node
         _xmlcfg_copy(m_xmlHandle, vcIndex, m_xmlHandle, defVC, VSXMLCFG_COPY_CHILDREN);
      }
   }
    ...
    * have made.
    *
    * @param saveChanges            whether to save any changes we may have made
    */
   public void closeExportDOM(boolean saveChanges = true)
   {
      if (m_xmlExportHandle > 0) {
    ...
    *
    * @param modeName      name of new language
    * @param userDef       index of user defined languages node
    * @param defLang       index of default language info node
    *
    * @return              index of new language
    */
   public int addNewLanguageToXML(_str langId, int userDef = -1, int defLang = -1, _str sysHelp = '')
   {
      ...
      if (defLang > 0) {
         ...
         // set help attributes
         _xmlcfg_add_attribute(m_xmlHandle, modeIndex, 'SystemHelp', sysHelp);
         _xmlcfg_add_attribute(m_xmlHandle, modeIndex, 'DialogHelp', '');

         // copy the default stuff into our new language node
         _xmlcfg_copy(m_xmlHandle, modeIndex, m_xmlHandle, defLang, VSXMLCFG_COPY_CHILDREN);
    ...
    */
   private boolean isLanguageNode(int index, boolean includeCategories = true)
   {
    ...
    */
   private boolean isVersionControlProviderNode(int index, boolean includeCategories = true)
   {
      // sometimes we do not allow the categories to be included - if this is
      // a category node, the this function will return false after we grab parent
      if (!includeCategories) {
   ...
   private void trimXML(int nodes[])
   {
      ...
      node := _xmlcfg_find_simple(m_xmlHandle, '//DefaultLanguageInfo');
      if (node) _xmlcfg_delete(m_xmlHandle, node);

      node = _xmlcfg_find_simple(m_xmlHandle, '//DefaultVersionControlInfo');
      if (node) _xmlcfg_delete(m_xmlHandle, node);
   }
   ...
   {
      groupIndex := _xmlcfg_find_simple(m_xmlExportHandle, "//ExportGroup[@Name='"groupName"']");
      if (groupIndex > 0) {
   ...
   public void deleteExportGroup(_str groupName)
   {
      ...
      index := _xmlcfg_find_simple(m_xmlExportHandle, "//ExportGroup[@Name='"groupName"']");
    ...
    */
   public void writeExportGroups(ExportImportGroup groups:[])
   {
      changed := false;
      ...
      foreach (auto group in groups) {
         ...
         if (group.isGroupModified()) {
            ...
            // see if we already have this group in xml
            groupIndex := _xmlcfg_find_simple(m_xmlExportHandle, "//ExportGroup[@Name='"group.getName()"']", topNode);
            if (groupIndex > 0) {
