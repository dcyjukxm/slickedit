...
class LanguageSettings {
   ...
   public static int getAllLanguageOptions(_str langID, VS_LANGUAGE_OPTIONS &langOptions)
   {
      ...
      if (langOptions.TruncateLength) {
         tr_index := _FindLanguageCallbackIndex('_%s_is_truncation_supported', langID);
         if (tr_index && !call_index(tr_index)) {
            langOptions.TruncateLength = 0;
         }
      }
   ...
   private static void setLanguageOptionItem(_str langID, int item, typeless value, boolean createNew = false)
   {
      ...
      if (optionsIndex <= 0) {
         if (createNew) {
            optionsIndex = insert_name('def-options-'langID, MISC_TYPE, DEFAULT_SYNTAX_INFO);
         } else {
            return;
         }
      }
    ...
    * @categories LanguageSettings_API 
    */
   public static _str getDiffColumns(_str langID, _str defaultValue = null)
   {
      return getLanguageDefVar(langID, DIFF_COLUMNS_DEF_VAR_KEY, defaultValue);
   }
   ...
   {
      setLanguageDefVar(langID, DIFF_COLUMNS_DEF_VAR_KEY, value);
   }
