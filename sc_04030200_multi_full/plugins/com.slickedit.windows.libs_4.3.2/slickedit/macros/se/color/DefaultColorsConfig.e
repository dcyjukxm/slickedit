...
static void loadScheme(_str name, boolean loadCurrentScheme=false) 
{
   ...
   if (scm != null &&
      ...
      ctl_symbol_scheme._lbsearch(scm->m_symbolColoringSchemeName);
      ctl_symbol_scheme.p_text = scm->m_symbolColoringSchemeName;
   } else {
...
static void loadColor(int cfg)
{
   ...
   if (ctl_system_default.p_enabled && ctl_system_default.p_value) {
      ctl_foreground_color.p_backcolor = VSDEFAULT_FOREGROUND_COLOR;
      ctl_background_color.p_backcolor = VSDEFAULT_BACKGROUND_COLOR;
      ctl_embedded_color.p_backcolor = VSDEFAULT_BACKGROUND_COLOR;
   } else {
...
void _color_form.on_create()
{
   ...
   if(!ctl_mode_name._lbsearch(_LangId2Modename("c"))) {
      ctl_mode_name.p_text=_LangId2Modename("c");
   }
...
void ctl_system_default.lbutton_up()
{
   ...
   boolean useSystemDefault = (p_value != 0);
