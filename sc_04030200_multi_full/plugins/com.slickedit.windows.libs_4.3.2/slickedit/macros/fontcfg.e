...
boolean _font_config_form_apply()
{
   ...
   for( i._makeempty();; ) {
      ...
      if (isEditorFontChange) {
         status=IDYES;
#if 0
         // User's get confused if we set the default font when
         // user responds "no" to the question "Change font in all windows?".
         if (already_prompted) {
            status=already_prompted_status;
         } else {
            status = _message_box(get_message(VSRC_FC_CHANGE_FONT_IN_ALL_WINDOWS),
                         '',
                         MB_YESNO|MB_ICONQUESTION);
            already_prompted_status=status;
...
void _ctl_element_list.on_change(int reason=CHANGE_OTHER)
{
   if( CHANGING_ELEMENT_LIST ) {
      ...
      _font_name_list._lbsearch(font_name);
