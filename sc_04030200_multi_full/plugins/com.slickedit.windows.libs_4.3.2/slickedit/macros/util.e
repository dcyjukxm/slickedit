 ...
 *  
 * @param rec_length For specifying a record length to use as a 
 *                   left hand margin.  Delete nothing before
 *                   the rec_length byte. 
 *  
 * @appliesTo Edit_Window, Editor_Control
 *
 * @categories Edit_Window_Methods, Editor_Control_Methods
 */
_command void remove_trailing_spaces(int rec_length=0) name_info(','VSARG2_MARK|VSARG2_REQUIRES_EDITORCTL)
{
   int hasselection = 0;
   int direction = -1;
   ...
   for (;;) {
      ...
      if (p_line != orig_line) {
         ...
         if (!status && match_length()) {
            if (rec_length > 0 && p_col < rec_length + 1) {
               int orig_col = p_col;
               _end_line();
               if (p_col > rec_length) {
                  int adjust = rec_length + 1 - orig_col;
                  p_col = rec_length + 1;
                  _delete_text(match_length()-adjust);
               } else {
                  p_col = orig_col;
               }
            } else {
