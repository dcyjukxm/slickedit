...
void _tbfind_form.rbutton_up()
{
   ...
   if (isEclipsePlugin()) {
      status := _menu_find(menu_handle, "tbDockableToggle", auto mh, auto mpos, 'M');
...
int mark_all_occurences(_str search_text, _str search_options, int search_range, int mfflags, int grep_id, boolean show_hilite, boolean list_all, boolean show_bookmarks,boolean scroll_markup=false)
{
   int status = 0;
   int num_matches = 0;
...
static void _lbadd_nodupe(_str text)
{
   int line = p_line;
   _str status = _lbsearch(text, 'E');
   p_line = line;
   if (status) {
      _lbbottom();
      _lbadd_item(text);
