...
_str _code_templates_export_settings(_str &path)
{
   ...
   if (file_exists(filePath)) {
      ...
      // copy the file over
      if (copy_file(filePath, path :+ fileName)) {
         error :+= 'Error copying formatting scheme file 'filePath'.' :+ OPTIONS_ERROR_DELIMITER;
      } else files :+= fileName',';
   }
   ...
   // get each file that exists here
   filePath = file_match("-du "maybe_quote_filename(templatesDir), 1);
   while (filePath != "") {
      ...
      filePath = file_match("-du "maybe_quote_filename(templatesDir), 0);
   }
...
static _str importCodeTemplateFile(_str templatesDir, _str path)
{
   ...
   if (file_eq(filename, CTOPTIONS_FILENAME)) {
      ...
      if (copy_file(path, templatesDir :+ filename)) {
         error = 'Error copying code template file 'filename'.' :+ OPTIONS_ERROR_DELIMITER;
      }
   }
...
_str _lang_diff_columns_property(_str langId, _str value = null)
{
   if (value == null) {
      ...
      LanguageSettings.setDiffColumns(langId, diffColOnOff' 'diffStartCol' 'diffEndCol);
   }
...
_str _vc_setup_executable(_str provID, _str value = null) 
{
   if (value == null) {
      switch (provID) {
      case 'CVS':
         if (def_cvs_info.cvs_exe_name == '' || !file_exists(def_cvs_info.cvs_exe_name)) {
         ...
         break;
      case 'Subversion':
         if (def_svn_info.svn_exe_name == '' || !file_exists(def_svn_info.svn_exe_name)) {
         ...
         break;
      case 'Git':
         if (def_git_info.git_exe_name == '' || !file_exists(def_git_info.git_exe_name)) {
...
/**
 * Gets/Sets the special character 'End of Line 1' setting.
 * 
 * @param value            current value to set, leave null to just return the 
 *                         current value
 * 
 * @return                 current value
 */
_str _special_characters_end_of_line1(_str value = null)
{
   return _special_characters_option(SC_END_OF_LINE_1, value);
}

/**
 * Gets/Sets the special character 'End of Line 2' setting.
 * 
 * @param value            current value to set, leave null to just return the 
 *                         current value
 * 
 * @return                 current value
 */
_str _special_characters_end_of_line2(_str value = null)
{
   return _special_characters_option(SC_END_OF_LINE_2, value);
}

/**
 * Gets/Sets the special character 'Tab' setting.
 * 
 * @param value            current value to set, leave null to just return the 
 *                         current value
 * 
 * @return                 current value
 */
_str _special_characters_tab(_str value = null)
{
   return _special_characters_option(SC_TAB, value);
}

/**
 * Gets/Sets the special character 'Space' setting.
 * 
 * @param value            current value to set, leave null to just return the 
 *                         current value
 * 
 * @return                 current value
 */
_str _special_characters_space(_str value = null)
{
   return _special_characters_option(SC_SPACE, value);
}

/**
 * Gets/Sets the special character 'Virtual Space' setting.
 * 
 * @param value            current value to set, leave null to just return the 
 *                         current value
 * 
 * @return                 current value
 */
_str _special_characters_virtual_space(_str value = null)
{
   return _special_characters_option(SC_VIRTUAL_SPACE, value);
}

/**
 * Gets/Sets the special character 'End of File' setting.
 * 
 * @param value            current value to set, leave null to just return the 
 *                         current value
 * 
 * @return                 current value
 */
_str _special_characters_end_of_file(_str value = null)
{
   return _special_characters_option(SC_END_OF_FILE, value);
}

/**
 * Gets/Sets the special character 'Form Feed' setting.
 * 
 * @param value            current value to set, leave null to just return the 
 *                         current value
 * 
 * @return                 current value
 */
_str _special_characters_form_feed(_str value = null)
{
   return _special_characters_option(SC_FORM_FEED, value);
}

/**
 * Gets/Sets a special character setting.
 * 
 * @param character        special character to get/set, one of the 
 *                         SpecialCharacters enum
 * @param value            current value to set, leave null to just return the 
 *                         current value
 * 
 * @return                 current value
 */
_str _special_characters_option(int character, _str value = null)
{
   current := _default_option(VSOPTIONZ_SPECIAL_CHAR_XLAT_TAB);
   if (value == null) {
      // get the specific character that we are interested in
      ch := substr(current, character, 1);
      ascii := _asc(ch);
      
      value = ch :+ ' ('ascii')';
   } else {
      _str ch, ascii;
      parse value with ch '(' ascii ')';
      newCurrent := substr(current, 1, character - 1) :+ _chr((int)ascii) :+ substr(current, character + 1);

      _default_option(VSOPTIONZ_SPECIAL_CHAR_XLAT_TAB, newCurrent);
   }
...
void initExportGroupsComboBox(_str selGroup)
{
   ...
   if (_ctl_export_groups._lbsearch(selGroup)) {
      // or just the top one
      _ctl_export_groups._lbtop();
   }
   _ctl_export_groups._lbselect_line();
   _ctl_export_groups.p_text = _ctl_export_groups._lbget_text();
}
...
void _ctl_new_export_group.lbutton_up()
{
   ...
   if (result == IDOK) {
      ...
      _ctl_export_groups._lbadd_item(groupName);
      _ctl_export_groups._lbsearch(groupName);
      _ctl_export_groups._lbselect_line();
      _ctl_export_groups.p_text = _ctl_export_groups._lbget_text();
...
void _ctl_cancel.lbutton_up(boolean cancelButton = true)
{
   ...
   if (purpose == OP_CONFIG || purpose == OP_QUICK_START) {
      ...
      if (optionsConfigTree -> areOptionsModified(true, true)) {
         ...
         /*
         buttons := "&Apply,&Discard Changes,Cancel:_cancel\t-html Changes have been made but not applied.";
         if (!cancelButton) {
            buttons = "&Apply,&Discard Changes\t-html Changes have been made but not applied.";
         }
         ...
         if (status == 1) {            // Apply - the first button
            if (!optionsConfigTree -> apply()) return;
         } else if (status == 2 || !cancelButton) {     // Discard Changes - the second button
            optionsConfigTree -> cancel();
...
{
   if (!_ctl_filter._lbsearch('Anytime')) {
      _ctl_filter._lbselect_line();
   }
   _ctl_filter.p_text = _ctl_filter._lbget_text();
}
...
void _options_history_form_restore_state()
{
   ...
   if (!_ctl_filter._lbsearch(_ctl_filter.p_text)) {
      _ctl_filter._lbselect_line();
   }
   _ctl_filter.p_text = _ctl_filter._lbget_text();
}
...
void _ctl_ok.lbutton_up()
{
   ...
   if (!_ctl_existing_groups._lbsearch(groupName, 'E')) {
