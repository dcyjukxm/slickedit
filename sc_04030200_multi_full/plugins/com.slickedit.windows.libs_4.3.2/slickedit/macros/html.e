...
{
   autoSymbolTransEditor(ctl_XW_autoSymbolTrans_button.p_user);
   return;
}
...
//   html_add_tld_globals(editorctl_wid, p_window_id, root_root);
   ...
   case '<':
      ...
      if (last_char(prefixexp)=='=') {
         ...
         if (isDTD) {
            typeless line_offset="";
            parse editorctl_wid.point() with line_offset .;
            w1 := w2 := w3 := text := "";  // text before the cursor
            if (line_offset<lastidstart_offset) {
               text=editorctl_wid.get_text_safe(lastidstart_offset-line_offset,line_offset);
            }
            if (tag_name=='!ELEMENT') {
