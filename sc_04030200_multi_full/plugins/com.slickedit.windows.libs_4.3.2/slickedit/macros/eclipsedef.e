...
#endif
   // turn off all automatic syntax expansion by default
   // yes, this means that if a user switches from Eclipse to another emulation
   // that syntax expansion will still be off
   setOptionForAllLanguages('SyntaxExpansion', false);
   // also turn off automatic alias expansion on space
   setOptionForAllLanguages('ExpandAliasOnSpace', false);
