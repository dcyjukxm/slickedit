...
static void jdCopyComboCtlData(int form_wid,_str prefix,_str ctlname,int flag,boolean &addBlankBeforeNext,_str tag)
{
      ...
      for (i=0;i<count;++i) {
         ...
         if (hashindex_tag!='param' || !def_javadoc_keep_obsolete ||
            ...
            if (LongestLen>=0 && !pos('<pre',rest:+list,1,'i') &&
               ...
               if (length(argName)>maxLen) {
                  insert_line(prefix:+'@'tag:+strip(' 'argName,'T'));
                  indent=substr('',1,length('@'tag)+LongestLen+2);
                  if (rest!='') {
                     insert_line(prefix:+indent:+strip(rest));
                  }
               } else {
                  indent=substr('',1,1+length('@'tag' ')+length(argName));
                  insert_line(prefix:+'@'tag:+strip(' 'argName:+' 'strip(rest),'T'));
               }
               doBeautify=true;
            } else {
               doBeautify=false;
               insert_line(prefix:+'@'tag' 'argName:+' 'rest);
            }
            for (;;) {
               if (list:=='') {
                  break;
 ...
 */
static void jdInsertCommentLines(int form_wid,int start_col)
{
   ...
   if (wid && wid.p_text!='') {
      ...
      for (;;) {
         ...
         insert_line(prefix'@author 'strip(author));
      }
   }
   wid=form_wid._find_control('ctlversion'form_wid.CURJAVATYPE);
   if (wid && wid.p_text!='') {
      insert_line(prefix'@version 'strip(wid.p_text));
   }
   jdCopyComboCtlData(form_wid,prefix,'ctlparamcombo',VSJAVADOCFLAG_BLANK_LINE_AFTER_PARAMETERS,addBlankBeforeNext,'param');
   jdCopyEditorCtlData(form_wid,prefix,'ctlreturn',VSJAVADOCFLAG_BLANK_LINE_AFTER_RETURN,addBlankBeforeNext,'@return ');
   jdCopyEditorCtlData(form_wid,prefix,'ctlexample',VSJAVADOCFLAG_BLANK_LINE_AFTER_EXAMPLE,addBlankBeforeNext,'@example ');

   jdCopyComboCtlData(form_wid,prefix,'ctlexceptioncombo',0,addBlankBeforeNext,'exception');

   jdCopyEditorCtlData(form_wid,prefix,'ctlsee',0,addBlankBeforeNext,'@see ',false);

   wid=form_wid._find_control('ctlsince'form_wid.CURJAVATYPE);
   if (wid && wid.p_text!='') {
      insert_line(prefix'@since 'strip(wid.p_text));
   }
   wid=form_wid._find_control('ctldeprecated'form_wid.CURJAVATYPE);
   if (wid) {
      // IF Deprecated check box is on
      if (wid.p_prev.p_value) {
         jdCopyEditorCtlData(form_wid,prefix,'ctldeprecated',0,addBlankBeforeNext,'@deprecated ');
      }
   }
   _str line="";
   get_line(line);
...
static void jdMaybeSave(boolean forceSave=false)
{
   if (jdModified() || forceSave) {
      ...
      jdInsertCommentLines(form_wid,start_col);
...
static void jdShowParam(_str tag='param')
{
   ...
   if (modify && isinteger(widcombo.p_user) && widcombo.p_user>=0) {
      ...
      if (length(text)) {
         ...
         widcombo.save_pos(p);
         widcombo._lbtop();
         typeless status=widcombo._lbsearch(param_name' (empty)');
         if (!status) {
...
_command void javadoc_editor(_str forceXMLDOC='') name_info(','VSARG2_REQUIRES_EDITORCTL)
{
