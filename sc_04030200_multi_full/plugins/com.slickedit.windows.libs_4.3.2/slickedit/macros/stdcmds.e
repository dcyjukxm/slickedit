...
definit()
{
   ...
   if ( initArg :!= 'L' ) {
      ...
      if ( _pic_cvs_filem>=0 ) {
         set_name_info(_pic_cvs_filem,"This file has been removed but not commited");
      }
...
void _cmdline_post_execute(_str text)
{
   if (def_unix_expansion) {
      /* Execute result of function call. */
      //_cmdline.set_command('',1);
...
_command void cmdline_toggle() name_info(','VSARG2_MARK|VSARG2_CMDLINE|VSARG2_READ_ONLY|VSARG2_REQUIRES_EDITORCTL|VSARG2_ICON)
{
   ...
   if ((p_window_id!=_cmdline &&
      ...
      */
      ...
      // move to post call to fix unintentional window deletion
      // from ArgumentCompletionTerminate
      _post_call(_cmdline_post_execute,text);
      return;
   }
...
static _str aboutLicenseInfo()
{
   ...
   _str license_name = '';
   if (isEclipsePlugin()) {
      license_name = "license_eclipse.txt";
   } else {
   ...
   // get the contents of the license file and convert it to HTML
   _str license_text = '';
   _GetFileContents(license_file, license_text);
   if (pos("\r", license_text)) {
      license_text = stranslate(license_text, "\r\n<p>\r\n", "\r\n\r\n");
   } else {
      license_text = stranslate(license_text, "\n<p>\n", "\n\n");
   }
   license_text = stranslate(license_text, "<b>\n#0\nIMPORTANT\n</b>", "^{?*}IMPORTANT", "er");
   license_text = stranslate(license_text, "<b>Additional Terms</b>", "Additional Terms[\r\n]", "er");
   license_text = stranslate(license_text, "<b>&reg;</b>", "\xae", "e");
   license_text = stranslate(license_text, "<b>&reg;</b>", "(R)", "e");
   license_text = stranslate(license_text, "&nbsp;", "\t", "e");
   license_text = stranslate(license_text, "<b>&ldquo;</b>", "\x{201C}", "e");
   license_text = stranslate(license_text, "<b>&rdquo;</b>", "\x{201D}", "e");
   license_text = stranslate(license_text, "<b><small><sup>TM</small></sup></b>", "(tm)", "e");
   //license_text = stranslate(license_text, "\n<b>CORP#0</b>\n", "\nCORP{?*}\n", "er");
   return license_text;
}
   ...
   if (def_auto_set_buffer_cache && p_buf_size>(def_auto_set_buffer_cache_ksize*1024)) { 
      ...
      if (isinteger(ksize) && ksize>=0 && ksize<recommended_ksize) {
         ...
         if (physical_memory_ksize) {
            /* 
                Only allow SlickEdit 1/2 of the physical memory. 
                Guess 400 megabytes for the rest of SlickEdit. 
            ...
            */ 
            ...
            if (!have_enough_available) {
               ...
               _str msg="You don't have enough memory for all editing operations. Add memory";
...
// }
   ...
   if (p_buf_size>def_use_undo_ksize*1024) {
      ...
      large_file_editing_msg:+="Undo turned off for better performance and to avoid filling up spill file";
      p_undo_steps=0;
   }
   if (large_file_editing_msg!='') {
...
int _OnUpdate_view_specialchars_toggle(CMDUI &cmdui,int target_wid,_str command)
{
   ...
   if ((p_ShowSpecialChars & (SHOWSPECIALCHARS_NLCHARS|SHOWSPECIALCHARS_TABS|SHOWSPECIALCHARS_SPACES|SHOWSPECIALCHARS_EOF|SHOWSPECIALCHARS_FORMFEED))==
       (SHOWSPECIALCHARS_NLCHARS|SHOWSPECIALCHARS_TABS|SHOWSPECIALCHARS_SPACES|SHOWSPECIALCHARS_EOF|SHOWSPECIALCHARS_FORMFEED)) {
      return(MF_CHECKED|MF_ENABLED);
   }
   return(MF_UNCHECKED|MF_ENABLED);
}
...
int _OnUpdate_view_formfeed_toggle(CMDUI &cmdui,int target_wid,_str command)
{
   ...
   if (p_ShowSpecialChars & SHOWSPECIALCHARS_FORMFEED) {
      return(MF_CHECKED|MF_ENABLED);
   }
   return(MF_UNCHECKED|MF_ENABLED);
}
...
static _str _parseCobolSource(
   _str firstword,
   _str line, _str sourceData)
{
   _str cobolword1, cobolword2;
   parse lowcase(substr(line,7)) with cobolword1 cobolword2 .;
   if ((isinteger(substr(line,1,6)) && cobolword1=='title') ||
       lowcase(firstword)=='title') {
      // skip current line
      _str rest;
   ...
   )
{
   ...
   _str status = _parseCobolSource(firstword, line, sourceData);
   if (status != '') {
      return status;
   }
   ...
   if (substr(firstword,1,1)=='#') {
       ...
       if (pos(' 'secondword' ',' include pragma define if ifdef ifndef ')) {
          return('c');
       }
   }
   if(substr(line,1,2)=='//' ){
...
void _UpdateEditorLanguage(int wid=0)
{
   ...
   //say('ext='ext);
   if ( wid.p_LangId != lang && lang != 'fundamental') {
...
void renumber_lines(int start_col,int end_col,_str pad_char='0',
                    boolean use_blanks=false, boolean quiet=false, boolean check_seq_num=false)
{
   typeless p;
   _save_pos2(p);
...
{
   return('MN='ModeName',TABS=+4,MA=1 74 1,KEYTAB=ext-keys,WW=1,IWT=0,ST=0,IN=2,WC=N/A,LN=,CF=0,LNL=0,TL=0,BNDS=,');
}
