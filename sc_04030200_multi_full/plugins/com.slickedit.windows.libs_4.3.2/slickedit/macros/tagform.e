   ...
   */
   ...
   while (!down()) {
      ...
      if (fdate!="" && fdate!=0) {
         ...
         if (doRetag) {
            if (!useThread || !file_exists(filename) || RetagCurrentFileAsync(filename) < 0) {
               boolean inmem;
...
ctlremove_tag_file.lbutton_up()
{
   ...
   // NOTE: this should not be done for an auto-updated tag file.  just remove it.
   if(FolderIndex != autoUpdateFilesIndex) {
      if (file_exists(TagFilename)) {
         result=_message_box(nls("Do you wish to delete the file %s?",TagFilename),'',MB_YESNOCANCEL|MB_ICONQUESTION);
      } else {
         result=IDNO;
      }
   }

   if(result==IDYES || FolderIndex == autoUpdateFilesIndex) {
...
static void reloadLangCompilers(int sectionIndex, _str langId, _str selection = '')
{
   ...
   newSectionIndex := _ctl_tagfiles._loadLangAutoTagChoices(langId, sectionIndex);
