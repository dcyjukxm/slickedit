                    ...
                    )
{
   ...
   _lbtop();
   typeless status=_lbsearch(CurConfig);
   if (status) {
      status=_lbsearch(ALL_CONFIGS);
      if (status) {
         _lbtop();
      }
   }
...
int _check_java_installdir(_str &install_dir, boolean check_for_jvm_lib = false)
{
   ...
   if (!file_exists(install_dir:+FILESEP:+'bin':+FILESEP:+'javac'
#if !__UNIX__
                    :+'.exe'
#endif
                    )) {
...
{
   _android_getRunArgs(auto sdk, auto apk, auto pkg, auto act, false);
   if (aOptions.androidLoc == sdk) {
      return true;
   }
   ...
   uCmd :+= '.bat';
#endif
   uCmd = maybe_quote_filename(uCmd) :+ ' update project --path 'projectDir;
...
static void SetAndroidValues()
{
   ...
   if (aOptions == null || aOptions.androidLoc=='') {
      _android_getRunArgs(auto sdk, auto apk, auto pkg, auto act, false);
...
void ctlandroid_sdk_loc.on_lost_focus()
{
   _str value=strip(p_text);
   android_options aOptions=ANDROID_OPTIONS;
   aOptions.androidLoc=value;
   ANDROID_OPTIONS=aOptions;
   ctlandroid_sdk_loc.p_sel_length = 0;
}
