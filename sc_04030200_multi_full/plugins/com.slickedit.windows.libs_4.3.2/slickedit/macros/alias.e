...
_str alias_filename(boolean includeExtAliasFile=true, boolean includeGlobalAliasFile=true)
{
   ...
   if (includeGlobalAliasFile) {
      ...
      if (filename=='') {
         filename=VSCFGFILE_ALIASES;
         //if (file_match('-p 'maybe_quote_filename(filename),1)=='') {
         //   filename=slick_path_search(VSCFGFILE_ALIASES);
         //}
      }
   }
               ...
               boolean quiet=false)
{
   ...
   typeless multi_line_flag='';
   int alias_linenum=0;
   int status=find_alias2(name,old_view_id,alias_view_id,
                          ...
                          )  name_info(','VSARG2_TEXT_BOX|VSARG2_REQUIRES_EDITORCTL)
{
   ...
   if (use_extension_specify_alias_file && p_HasBuffer) {
      aliasfilename=_get_lang_alias_filename();
   }
#endif
   int old_col=0;
   if (command_state()) {
 ...
 * @return int
 */
static int expand_alias2(_str name,
                         _str first_line,
                         int start_col,
                         ...
                         boolean isAutoExpansion = false)
{
   ...
   int aliasHasParams = pos('(',true_name);
   if ( aliasHasParams ) {
   ...
   if(!dest_utf8) line=_UTF8ToMultiByte(line);

   typeless alias_col='';
   int blank_col=pos(' ',line);
   if ( blank_col ) {
   ...
   //It does not appear that alias_col will ever be '' based on how the
   //alias files are written when stored.  DOB 11/01/07
   ...
   for (;;) {
      ...
      if(!dest_utf8) alias_line=_UTF8ToMultiByte(alias_line);
 ...
 * @return void 
 */
static void cancel_expand_alias_line(typeless expand_first, int start_col, _str originalLine) {
   if (!expand_first) {
      delete_line();
      up();
   } else {
...
static int build_prompts(int &prompt_view_id, int &var_view_id)
{
   int view_id=0;
   get_window_id(view_id);
...
int has_multi_line_code(_str alias)
{
   if ( pos('{%(\\[]|\([0-9a-zA-Z_]#\))}',alias,1,'r') ) {
      return 1;
   }
   return(0);
}

static int delete_multi_line_alias()
{
   for (;;) {
      _delete_line();
...
static int _delete_alias(_str name)
{
   ...
   typeless multi_line_flag='';
   int alias_linenum=0;
   int find_status= find_alias2(name,old_view_id,
                                alias_view_id,multi_line_flag,
                                aliasfilename,alias_linenum,false);
   ...
   int status=save('',SV_OVERWRITE);
   if ( status ) {
      activate_window(old_view_id);
      return(status);
   }
   _delete_temp_view(alias_view_id);
   activate_window(old_view_id);
...
static _str _get_lang_alias_filename()
{
   ...
   return getAliasFileNameForLang(lang);
}
 ...
 * </p>
 * 
 * <p>Aliases are stored by default in a file called "alias.slk".  Use the VSLICKALIAS environment
 * variable to define one or more alias files.  Separate each alias filename with a ';' (UNIX: ':').
...
_command int alias(_str params='') name_info(ALIAS_ARG',')
{
   ...
   typeless multi_line_flag=0;
   typeless delete_option='';
   typeless rest='';
   typeless status2='';
   ...
   if (delete_option!='') {
      return(_delete_alias(name));
            ...
            */
   aliasfilename=next_alias_file(alias_filename(false),1);
   #else
   aliasfilename='';
   #endif
   int old_view_id=0;
   ...
   if ( content=='') {
      ...
      if (find_status==FILE_NOT_FOUND_RC || find_status==STRING_NOT_FOUND_RC) {
         ...
         status2=_edit_first_alias_file(alias_view_id,'',
                                        true  /*Clark: directory alias file only*/);
...
static int find_alias2(_str name,
                       ...
                       )
{
   ...
   if ( filename=='' ) {
      ...
      if (!ignore_not_found) {
         temp_install_alias_file=get_env('VSROOT'):+'$alias.slk';
   ...
   for (;;) {
      alreadyOpen = false;
      status=_open_temp_view(filename,alias_view_id,junk, "", alreadyOpen, false, false, more_buf_flags);
      if (status) {
         if (!quiet) {
            if ( status==FILE_NOT_FOUND_RC ) {
                if (!ignore_not_found) {
                   temp_install_alias_file=get_env('VSROOT'):+'$alias.slk';
                   if (file_match2(temp_install_alias_file,1,'-p')=='') {
      ...
      if (alreadyOpen && more_buf_flags) {
         int aviBufID = p_buf_id;
         int new_alias_view_id;
         _create_temp_view(new_alias_view_id);
         _buf_transfer(aviBufID);
         _delete_temp_view(alias_view_id);
         alias_view_id = new_alias_view_id;
      }
      top();
      status=search('^'_escape_re_chars(name)'( |$|\()','r@'def_alias_case);
      if ( ! status ) {
         ...
         multi_line_flag = pos(temp,'(') || has_multi_line_code(old_content);
         if ( ! multi_line_flag ) {
...
int find_alias_completions(var words, boolean forceUpdate=false)
{
   ...
   while (filename!='') {
      _str match_name = alias_match2(filename, alias_name, 1, alias_view_id, old_view_id);
      while (match_name != '') {

         // now attempt to find the specific alias
         activate_window(alias_view_id);
         _str line='';
         get_line(line);
         _str expanded_alias = '';
         parse line with . expanded_alias;
         if ( expanded_alias == "" ) {
            _delete_temp_view(alias_view_id);
            p_window_id=old_view_id;
            return STRING_NOT_FOUND_RC;
         ...
         while (!down()) {
            ...
            get_line(line);
            strappend(comments, p_newline);
            strappend(comments, strip(line));
         }
         restore_pos(p);

         if (comments != '') {
            comments = "<pre>" :+ expanded_alias :+ comments :+ "</pre>";
         ...
         // next please
         match_name = alias_match2(filename, alias_name, 0, alias_view_id, old_view_id);
      }
...
static int _edit_first_alias_file(int &alias_view_id, _str unused, boolean includeExtAliasFilename=false)
{
   // Do not get extension specific alias
   _str filename='';
   parse alias_filename(includeExtAliasFilename) with filename (PATHSEP) .;
   if ( filename=='') {
      return(1);
   }
...
static _str alias_match2(_str filename,_str name,int find_first,int &temp_view_id,int &orig_view_id)
{
   int status=0;
   if ( find_first ) {
      if ( find_first==2 ) {
      ...
      status=_open_temp_view(filename,temp_view_id,orig_view_id,'+d');
      if ( status) {
         if ( status==FILE_NOT_FOUND_RC ) {
            // Alias files could be deleted.  The caller could do cleanup to
            // some how remove the reference.
            sticky_message(nls("Alias file '%s' not found.",filename));
         } else {
            sticky_message(nls("Error loading alias file '%s'.",filename));
            //orig_view_id=_create_temp_view(temp_view_id,'',filename);
         }
         return('');
      }
      if ( name=='' ) {
         ...
         status=search('^'_escape_re_chars(name),'@ri');
      }
   } else {
   ...
   for (;;) {
      ...
      if ( strieq(name,part) && def_alias_case:!='e' ) {
         break;
      }
      status=repeat_search();
   }
...
_str _check_alias_name(_str name,int wid=0)
{
   if (!isid_valid(name)) {
   ...
   typeless multi_line_info="";
   _str alias_info = p_window_id.get_alias(name, multi_line_info, '', '', true);
   if (alias_info :!= '') {
...
_command int new_alias(_str name="", boolean showInAliasEditor=true) name_info(','VSARG2_MARK|VSARG2_REQUIRES_EDITORCTL|VSARG2_REQUIRES_AB_SELECTION)
{
   ...
   if (name == "") {
      status = textBoxDialog("Enter New Alias Name", 0, 0, "Enter New Alias Name dialog", "", "", 
                           "-e _check_alias_name:"p_window_id" Alias name:");
      if (status < 0) {
         return status;
      }
      name = _param1;
   }
   if (_check_alias_name(name, p_window_id) != 0) {
      return INVALID_ARGUMENT_RC;
   }
   ...
   int alias_view_id=0;
   int orig_view_id=0;
   status=_open_temp_view(alias_filename,alias_view_id,orig_view_id);
   if (status) {
      orig_view_id=_create_temp_view(alias_view_id,'',alias_filename);
      if (orig_view_id < 0) {
   ...
   // now insert the name of the new alias
   p_window_id=alias_view_id;
   bottom();
   ...
   do {
      ...
      if (p_line == start_line) {
         ...
         _insert_text(substr('', 1, length(name)+1));
      }
   } while ( !down() );

   // save the modified alias file
   status=_save_config_file(alias_filename);
   if (status) {
      _message_box(nls("Could not save file %s.\n\n%s",alias_filename,get_message(status)));
      _delete_temp_view(alias_view_id);
      p_window_id=orig_view_id;
      return(status);
   }
