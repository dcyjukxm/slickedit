...
_command void eclipse_make_tags(_str params='') name_info(',')
{
   _str bg = _is_background_tagging_enabled(AUTOTAG_WORKSPACE_NO_THREADS) ? "-B " : "";
   make_tags(bg:+params);
}

_str eclipse_get_beautifier_profiles(int wid=0)
{
   _str profsArray[];
   _str profs = '';
   profsArray._makeempty();
   if (wid && wid._isEditorCtl()) {
      profsArray = beautifier_profiles_for(wid.p_LangId);
      for (i := 0; i < profsArray._length(); i++) {
         profs :+= profsArray[i]',';
      }
   }
   return profs;
...
_str eclipse_get_beautifier_profile(int wid=0)
{
   _str prof = '';
   if (wid && wid._isEditorCtl()) {
      prof = LanguageSettings.getBeautifierProfileName(wid.p_LangId);
   }
   return prof;
}

// wholesaled from _OnUpdate_beautify
boolean eclipse_is_beautifier_supported(int wid=0)
{
   if ( !wid || !wid._isEditorCtl() ) {
      return false;
   }

   _str lang=wid.p_LangId;
   if ( lang!="" && lang!="fundamental" && 
        (BeautifyCheckSupport(lang) && !new_beautifier_supported_language(wid.p_LangId)) ) 
      return false;

   return true;
}

//wholesaled from _OnUpdate_beautifer_edit_current_profile
boolean eclipse_is_new_beautifier_supported(int wid=0)
{
   if ( !wid || !wid._isEditorCtl() ) {
      return false;
   }

   if (new_beautifier_supported_language(wid.p_LangId)) {
      return true;
   } else {
      return false;
   }
