...
defload()
{
   ...
   _str setup_info1985 = 'MN='COBOL_MODE_NAME',TABS=1 8 250 +4,MA=8 72 8,':+
                         'KEYTAB=cobol_keys,WW=1,IWT=0,ST=0,IN=2,':+
   ...
   if (LanguageSettings.getAliasFilename(COBOL_LANGUAGE_ID) == '') {
      LanguageSettings.setAliasFilename(COBOL_LANGUAGE_ID, COBOL_LANGUAGE_ID'.als');
   }
   if (LanguageSettings.getAliasFilename(COBOL74_LANGUAGE_ID) == '') {
      LanguageSettings.setAliasFilename(COBOL74_LANGUAGE_ID, COBOL_LANGUAGE_ID'.als');
   }
   if (LanguageSettings.getAliasFilename(COBOL2000_LANGUAGE_ID) == '') {
      LanguageSettings.setAliasFilename(COBOL2000_LANGUAGE_ID, COBOL_LANGUAGE_ID'.als');
   }
...
#define COBOL_VERBS (' accept add allocate alter call cancel ':+\
               ...
               'generate go goback if initialize initiate inspect invoke is ':+\
...
static _str cobol_expand_space()
{
   ...
   _str word=min_abbrev2(orig_word,words,name_info(p_index),aliasfilename);
   ...
   if ( word=='accept' ) {
      ...
      _str hitlist[];
      hitlist[0]='DATA DIVISION';
      hitlist[1]='DATA RECORD';
      ...
      result=strip(result);
      if (pos('RECORD', result) > 0) {
         replace_line(linenum_space:+_word_case('data'):+' ':+_word_case('record'):+' ':+_word_case('is'):+' ');
         p_col=text_col(leading_space)+16;
      } else if (pos('DIVISION', result) > 0) {
         leading_space=division_indent();
         linenum_space=substr(linenum_space,1,length(leading_space));
   ...
   if (filter_flags != 0) {
      if (otherinfo!=null && otherinfo!='') {
         ...
         case 'CALL':
            ...
            if (!num_matches && lastid_noquotes != "") {
               tag_list_any_symbols(0, 0, lastid_noquotes, tag_files,
                                    VS_TAGFILTER_PACKAGE, context_flags,
                                    num_matches, max_matches,
                                    exact_match, case_sensitive);
            }
            break;
         ...
         case 'OF':
            ...
            if (num_matches == 0) {
               tag_list_in_class(lastid,cur_class_name,
                                 ...
                                 null,null, visited, depth);
               if (lastid_noquotes != "") {
                  tag_list_in_class(lastid_noquotes,cur_class_name,
                                    0, 0, tag_files,
                                    num_matches, max_matches,
                                    filter_flags,context_flags,
                                    exact_match,case_sensitive,
            ...
            if (num_matches == 0) {
               pkg_class_name := cur_class_name:+VS_TAGSEPARATOR_package:+rt.return_type;
               tag_list_in_class(lastid,pkg_class_name,
                                 0, 0, tag_files,
                                 num_matches, max_matches,
                                 filter_flags,context_flags,
                                 exact_match,case_sensitive,
                                 null,null, visited, depth);
               if (lastid_noquotes != "") {
                  tag_list_in_class(lastid_noquotes,pkg_class_name,
                                    0, 0, tag_files,
                                 num_matches, max_matches,
                                 filter_flags,context_flags,
                                 exact_match,case_sensitive,
...
*/
void _cob_indent_surround(_str direction, int (&indents):[])
{
