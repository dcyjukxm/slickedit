...
{
   _android_getRunArgs(auto sdk, auto apk, auto pkg, auto act);
   if (sdk == '') {
...
void _android_update_form.on_create(_str sdk="", _str man_file="")
{
   if (sdk != "") {
...
void a_cancel.lbutton_up()
{
   ...
   int status=(*pfnCancelButton)("", "", "", man_file);
}
...
// int setupCpath = ctlcpath.p_value;
   ...
   int status=(*pfnOkButton)(name, target, sdk, man_file, true);
}
...
void android_ok.lbutton_up()
{
   ...
   int status=(*pfnOkButton)(name, package, activity, target, sdk, tagsdk, setupCpath, lib, apiLevel);
...
static int setupExistingAndroidProject(_str name='', _str target_id='', _str sdk='', _str man_file='', boolean update=false)
{
   ...
   _str workspaceName = _strip_filename(xmlFile, 'N') :+ wsname :+ WORKSPACE_FILE_EXT;
   ...
   if (workspaceCreated) {
      ...
      _android_addWildcardsToProject(projectHandle);
 ...
 * @param handle 
 */
void _android_addWildcardsToProject(int handle=0)
{
   if (handle > 0) {
...
_command void android_runAppOnDevice()
{
   _android_getRunArgs(auto sdk, auto apk, auto pkg, auto act);
   if (sdk == '') {
      _message_box("Error: No Android SDK specified in local.properties for project.");
   ...
   _str val = show("-xy -modal _android_device_form", emus);
   if (val == "") {
      // ? command cancelled...no device chosen
      return;
   }
...
int _android_getRunArgs(_str &sdk, _str &apk, _str &pkg, _str &act, boolean maybe_quote_sdk = true)
{
   _str projectDir = _file_path(_project_name);
   _maybe_append_filesep(projectDir);
...
_command int workspace_open_android(_str xmlFile = "")   name_info(FILE_ARG'*,')
{
   if (xmlFile == "") {
   ...
   int status=show('-modal _android_update_form',sdk, xmlFile);
   if (status) {
      if (status=='') {
         return(COMMAND_CANCELLED_RC);
      } else {
...
static _str _android_getNumberFromTarget(_str &target)
{
