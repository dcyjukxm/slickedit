...
void filter_init(boolean doRaw=false)
{
  ...
  if (mtype=="BLOCK" && trunc_length && lastcol>trunc_length) {
     if (lastcol>trunc_length) {
        lastcol=trunc_length;
     }
     if (firstcol>trunc_length) {
        firstcol=trunc_length;
        lastcol=trunc_length-1;
     }
  }
...
_str filter_get_string(_str &string)
{
   ...
   int trunc_length = _TruncateLengthC();
   if (trunc_length && grightcol>trunc_length) {
      grightcol=trunc_length;
      if (_leftcol>trunc_length) {
         _leftcol=trunc_length;
         grightcol=trunc_length-1;
      }
   }
                   ...
                   int (&indents):[]=null)
{
   ...
   _str line=indent_string(indent_width):+substr(s,non_blank); 
   if (!indents._indexin(line)) {
      // save how much we indented the line
      indents:[line]=syntax_indent-adjust;
   }
   return(line);
 ...
 * @categories Edit_Window_Methods, Editor_Control_Methods
 */
_command void unindent_line(int (&indents):[] = null) name_info(','VSARG2_MARK|VSARG2_REQUIRES_EDITORCTL)
{
   _str line="";
   ...
   if (p_TruncateLength > 0 && indents != null && indents._indexin(line)) {
      // if line truncation is on, use the saved indent (if any)
      indent = indents:[line];
   }
 ...
 * @categories Edit_Window_Methods, Editor_Control_Methods
 */
_command void indent_line(int (&indents):[]=null) name_info(','VSARG2_MARK|VSARG2_REQUIRES_EDITORCTL)
{
   // since the user called this explicitly, we assume they want 
   // to indent a blank line
   _indent_line(true,indents);
}
 ...
 */
void _indent_line(boolean forceIndentEmptyLine, int (&indents):[]=null)
{
   _str line="";
   if (p_TruncateLength > 0) {
   ...
   line=reindent_line(line, p_SyntaxIndent, forceIndentEmptyLine, indents);
   replace_line(line);
   first_non_blank();
}
