...
#if __MACOSX__
boolean def_mac_hide_mdichild_title = true;
#endif
#if __UNIX__
...
void _firstinit()
{
   ...
   if (bool && !cant_write_config_files) {
      ...
      if (isEclipsePlugin()) {
         ...
         if (r != '') {
            ...
            _str bmr = r_t :+ "bitmaps" :+ FILESEP;
            _str incr = r_t :+ "macros" :+ FILESEP;
            _str misc = r_t;
            if (!(r_t :== r)) {
               bmr = maybe_quote_filename(bmr);
               incr = maybe_quote_filename(incr);
               misc = maybe_quote_filename(misc);
            }
            set_env("VSLICKBITMAPS",bmr);
            set_env("VSLICKINCLUDE",incr);
         }
      }
   ...
   if (index_callable(find_index("_InitializeFileNameMapping", PROC_TYPE))) {
      _post_call(_InitializeFileNameMapping);
   }
}
 ...
 * files such as "vslick.sta" (UNIX: "vslick.stu") and "alias.slk".
 * 
...
void _asm390_update_settings()
{
   ...
   if (_version_compare('17.0.3.1', LanguageSettings.getUpdateVersion('asm390')) > 0) {
      updateKeytab('asm390_keys', 'TAB', 'cob-tab', '');
      updateKeytab('asm390_keys', 'S-TAB', 'cob-backtab', '');
   }
...
static void change_def_macfiles_location(_str old_default_config, _str new_default_config)
{
   ...
   foreach (auto macroFile in def_macfiles) {
      ...
      macroDir := _strip_filename(macroFile,'N');
      macroConfigDir := substr(macroDir,1,length(old_default_config));
      rest_of_path   := substr(macroFile,length(macroConfigDir));
      if (macroConfigDir == old_default_config) {
         macroFile = new_default_config :+ rest_of_path;
      }
