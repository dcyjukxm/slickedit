...
void maybe_refresh_lexer_list()
{
   ...
   for (i := 0; i < lexers._length(); i++) {
      ...
      if (_ctllexer_list._lbsearch(lexer)) {
...
void _cc_form_init_for_options(_str langId)
{
   ...
   // if we can't find the name, just select the top thing.
   if (!_ctllexer_list._lbsearch(lexer_name)) {
...
_ctlok.on_create(_str lexer_name='', int initial_tab=-1)
{
   ...
   if (lexer_name!='' && lexer_name!='fundamental') {
      status=_ctllexer_list._lbsearch(lexer_name);
      if (status) {
...
static void AddNewLexer(_str LexerName, _str copyFromLexer = '')
{
   ...
   save_pos(auto p);
   _lbtop();_lbup();
   typeless status=_lbsearch(LexerName,'i');
   //p_window_id=wid;
   if (!status) {
   ...
   _ctllexer_list.p_text=_lbget_text();
   AddLexerToModList();
   p_window_id=wid;
}
   ...
   while (result != '') {
      ...
      if (loadlexer == IDYES) {
         ...
         if (!status) {
            ...
            _delete_temp_view(lexer_view_id);
            if (_ctllexer_list._lbsearch(lexername)) {
               _ctllexer_list._lbadd_item(lexername);
               _ctllexer_list._lbsort('i');
            }
            p_window_id = orig_wid;
...
static void cc_prepare_ml_comment(COMMENT_TYPE comment,_str lexername,int commentnum)
{
   ...
   _ctlcolor_name._lbsearch(color,'i');
   _ctlcolor_name.p_text=_ctlcolor_name._lbget_text();
