 ...
 * @return int Zero on success
 */
EXTERN_C int VSAPI _InsertXcodeProjectFileList(VSPSZ projectPath, int windowID, bool absPaths);
...
/**
 * Returns all the schemes defined for an Xcode workspace 
 * (.xcworkspace bundle). Only available on the Mac. No-op on 
 * other platforms. 
 * @param workspacePath Full path to .xcworkspace bundle
 ...
 * @return int Zero on success
 */
EXTERN_C int VSAPI _InsertVCXProjectFileList(VSPSZ projectFilePath, int xmlHandle, int viewListID, bool absPaths, bool indentSpace);
