...
typedef void (VSAPI *VSPFNCALLBACK_AR_BUF_LINES_INSERTED2)(int buf_id,int AfterLineNum,int Noflines,int SplitLineCase);
typedef void (VSAPI *VSPFNCALLBACK_AR_BUF_LINES_DELETED)(int buf_id,int FirstLineNum,int Noflines);
typedef void (VSAPI *VSPFNCALLBACK_AR_BUF_LINES_DELETED2)(int buf_id,int FirstLineNum,int Noflines,int JoinLineCase);
typedef void (VSAPI *VSPFNCALLBACK_AR_BUF_DRAW_LINE)(int buf_id, int wid, int y, int height, int LineNum,int RealLineNum, int PastBottomOfFile, int SoftWrap);
...
typedef void (VSAPI *VSPFNCALLBACK_AR_BUF_DELETE_TEXT)(int buf_id,long StartOffset,long EndOffset);
typedef void (VSAPI *VSPFNCALLBACK_AR_BUF_INSERT_TEXT)(int buf_id,long Offset,long NofBytes);
typedef void (VSAPI *VSPFNCALLBACK_AR_BUF_REPLACE_TEXT)(int buf_id,long Offset,long InsertNofBytes,long DeleteNofBytes);
...
#define VSSHOWSPECIALCHARS_FORMFEED 0x10
...
int VSAPI vsBookmarkAdd(const char *pszBookmarkName,
                         ...
                         int RealLineNumber VSDEFAULT(-1),
                         int col VSDEFAULT(0),
                         long BeginLineROffset VSDEFAULT(0),
...
int VSAPI vsBookmarkGetInfo(int i,
                            ...
                            int *pRealLineNumber VSDEFAULT(0),
                            int *pCol VSDEFAULT(0),
                            long *pBeginLineROffset VSDEFAULT(0),
...
void VSAPI vsMiniHTMLSetDefaultFixedFont(char *pszFontName,int charset);
void VSAPI vsMiniHTMLSetDefaultFixedFontSize(int size /* 0..7 */,int PointSizeX10);
void VSAPI vsMiniHTMLSetDefaultProportionalFont(char *pszFontName,int charset);
 ...
 */
int VSAPI vsBufLineNumToRealLineNum(int buf_id,int LineNum);
/**
 ...
 *
 */
int VSAPI vsBufGetNofLines(int buf_id);
/**
 ...
 *
 */
int VSAPI vsBufGetNofNoSaveLines(int buf_id);
/**
 ...
 *
 */
int VSAPI vsBufGetLineNumFromOffset(int buf_id,long Offset,int *pRLineNum VSDEFAULT(0));
...
#define VS_LANGUAGE_OPTIONS_VERSION 6
...
#define VSCOLORFLAG_CURRENT_LINE  0x4
   ...
   int ValueCasing;
   int HexValueCasing;
   // version = 8       // version 7 was not reflected in C code
   int HexMode;
   int LineNumbersLen;
   // version = 9
 ...
 *
 * @param piLine pointer to int variable where the line number will be stored.
...
#define VSLTF_BEAUTIFIER            0x10000  // Set when this is associated with a beautifier job.
 ...
 *
 */
int VSAPI vsHaveSelection(int wid,int reserved VSDEFAULT(0));
/**
 * @return
 * <dl>
 ...
 *
 */
int VSAPI vsGetSelInfo(int wid,int markid,int *pfirstcol,int *plastcol VSDEFAULT(0),int *pbuf_id VSDEFAULT(0),char *pszBufName VSDEFAULT(0),int MaxBufName VSDEFAULT(0),int *pMaxBufNameLen VSDEFAULT(0));
/**
 ...
 *
 */
int VSAPI vsGetSelLines(int wid,int markid,int *pfirstline,int *plastline VSDEFAULT(0));
/**
 ...
 *
 */
int VSAPI vsQLineNumberFromReal(int wid,int RealLineNumber);
/**
 ...
 *
 */
int VSAPI vsQLineNumberFromOffset(int wid,VSINT64 offset);
/**
 * @return Returns line number (<B>VSP_LINE</B>) corresponding to
 * <i>OldLineNumber</i>.
 *
 ...
 *
 */
int VSAPI vsQLineNumberFromOld(int wid,int OldLineNumber);
/**
 ...
 *
 */
int VSAPI vsQRealLineNumberFromLine(int wid,int LineNumber);
/**
 ...
 *
 */
int VSAPI vsQOldLineNumberFromLine(int wid,int LineNumber);
/**
...
/**
 * <PRE>
 * void (VSAPI *VSPFNUPDATEEDITORSTATUS)(int wid,int RefreshFlags,int linenum,
...
/**
 * <PRE>int (VSAPI *pfnQDebugBitmap)(int buf_id,int LineNum,int RealLineNum,int OldLineNum,int wid);</PRE>
...
/**
 * <PRE>int (VSAPI *pfnSetLineColor)(int buf_id,int wid, int linenum,int LineFlags,
 ...
 * <pre>
 * #include <vsapi.h>
 * void VSAPI CallbackTextChange(int buf_id, int wid, long StartMod, long
 * EndMod) {
 *      VSSAVEPOS  pos;
 ...
 *      vsUp(wid,1);
 *      int linenum;
 *      linenum=vsPropGetI(wid,VSP_LINE);
 *      if (linenum==0 ) {
 ...
 *      int pass=1;
 *      for (;;) {
 *           long StartOffsetOfLine,down_count;
...
/**
 * <PRE>int (VSAPI *pfnQDefaultBitmap)(int wid,int LineNum,int RealLineNum,int OldLineNum,int reserved);</PRE>
...
enum VSCALLBACK_AR {
/**
 * <PRE>void (VSAPI *pfnInsertLine)(int buf_id,int AfterLineNum,int Noflines)</PRE>
...
/**
 * <PRE>void (VSAPI *pfnDeleteLine)(int buf_id,int FirstLinenum,int Noflines)</PRE>
...
/**
 * void (VSAPI *pfnTextChange)(int buf_id,int wid, long StartMod,long EndMod);
 *
...
/*
   The VSCALLBACK_AR_BUF_INSERT_TEXT callback was dreprecated because the
...
/**
 * <PRE>void (VSAPI *pfnReplaceText)(int buf_id,long StartOffset,long InsertNofBytes,long DeleteNofBytes)</PRE>
...
/**
 * <PRE>void (VSAPI *pfnInsertLine)(int buf_id,int AfterLineNum,int Noflines,int SplitLineCase)</PRE>
 * <p>The REPLACE_TEXT callback is called when any text is replaced
 * into a buffer.</p>
 *
 * <p>WARNING:  It is not safe to access editor buffer data during this
...
/**
 * void (VSAPI *pfnDeleteLine)(int buf_id,int FirstLinenum,int Noflines,int JoinLineCase)
 * <p>The LINES_INSERTED callback is called when lines are inserted into
 * a buffer.</p>
 *
 * <p>WARNING:  It is not safe to access editor buffer data during this
...
/**
 * void (VSAPI *pfnDrawLine)(int buf_id, int wid, int y, int height, int LineNum,int RealLineNum, int PastBottomOfFile, int SoftWrap)
 * <p>
 ...
 *
 */
void VSAPI vsSetTextColor(int wid,VSCOLORINDEX ColorIndex,int ColorLen,bool replacePreviousColor=true);
...
#define VSCFG_MODIFIED_FILE_TAB          48
 ...
 *
 */
void VSAPI vsBufName2Caption(int wid,char *pszCaption,int MaxCaptionLen,char state,int *pMaxCaptionLen VSDEFAULT(0));
/**
...
#define  VSBDS_FIXED_SINGLE  1
    ...
    *
    */
   int VSAPI vsQTCreateEditorCtl(int wid,QWidget *pParentWidget,int x,int y,int width,int height,
                               int BorderStyle VSDEFAULT(VSBDS_FIXED_SINGLE),
                               int visible VSDEFAULT(1),
...
/**
 * This function is called after an editor window catches a QDropEvent
...
void VSAPI vsDropFiles(slickedit::SEArray<slickedit::SEString> dropFiles);
...
*/
   ...
   /*
       PARAMETERS
         ...
         area        One of the VSDOCKINGAREA_??? constants indicate the area
                     a window is/was docked.
         change      one of the following
                     'A'   Window docked
                     'D'   Docked window deleted
                     'S'   Docked window size changed
       Remarks
   ...
   */
   void (VSAPI *pfnMDIDockingAreaChange)(int mdi_wid, vsDockingArea area, char change);
#endif
   ...
   /*
       ...
       files are effected:
           ...
           win\vslick.vsb       (UNIX: bin/vslick.vsb): When a non-null pszLang is
                                specified, the default vslick.vsm is not used.
                                Instead "vslick_<Lang>.vsb" is used.

           vslick.vsm           When a non-null pszLang is
                                specified, the default vslick.vsm is not used.
                                Instead "vslick_<Lang>.vsm" is used.
                                When installing these files make sure the ".vsb" file
                                is newer than the ".vsm" file so a macro compile is
 ...
 *
 */
long VSAPI vsMatchStart(int TaggedExpression VSDEFAULT(-1));
/**
 ...
 *
 */
int VSAPI vsQSelectType(int wid,int markid VSDEFAULT( -1),char option VSDEFAULT('T' /* T S P I U W*/));
/**
 ...
 *
 */
int VSAPI vsGetRText(int wid,int Nofbytes,long point,char *pszBuf,int *pNofbytesRead VSDEFAULT(0));
/**
 ...
 *
 */
void VSAPI vsGoToOldLineNumber(int wid,int OldLineNum,int Reserved VSDEFAULT(1));
/**
 ...
 *
 */
int VSAPI vsGoToLine(int wid,int LineNumber,int OnlyCountRealLines VSDEFAULT(1),int doCenterScroll VSDEFAULT(1));
/**
 ...
 *
 */
int VSAPI vsGoToPoint(int wid,long Point,long DownCount VSDEFAULT(0),int LineNum VSDEFAULT((-1)));
/**
 * @param wid	Window id of editor control.  0 specifies the
 * current object.
 *
 ...
 *
 */
void VSAPI vsQPoint(int wid,long *pPoint,long *pDownCount,char Option VSDEFAULT('P'));
/**
 ...
 *
 */
int VSAPI vsSearch(int wid,const char *pSearchString,int SearchStringLen, const char *pszOptions VSDEFAULT(0),const char *pReplaceString VSDEFAULT(0),int ReplaceStringLen VSDEFAULT(0),int *pNofchanges VSDEFAULT(0));
int VSAPI vsSearchRaw(int wid,const char *pSearchString,int SearchStringLen, const char *pszOptions VSDEFAULT(0),const char *pReplaceString VSDEFAULT(0),int ReplaceStringLen VSDEFAULT(0),int *pNofchanges VSDEFAULT(0));
/**
...
VSPSZ VSAPI vsLCQDataAtLine(int wid,int LineNum);
...
int VSAPI vsLCQFlagsAtLine(int wid,int LineNum);

int VSAPI vsLCSetFlagsAtLine(int wid,int LineNum,int flags,int mask VSDEFAULT(-1));
 ...
 */
VSPICLISTITEM *VSAPI vsPicFindList(int wid,int LineNum,long LineOffset=VSNULLSEEK,int CheckNofLines=0,int reserved=0);
...
int VSAPI vsPicGet(int PicIndex,
                   int *pRealLineNum VSDEFAULT(0),
                   int *pLineNum VSDEFAULT(0),
                   int *pNofLines VSDEFAULT(0),
 ...
 * int LineMarkerIndex=vsLineMarkerAdd(0, vsPropGetI(0,VSP_LINE),
 ...
 *
 */
int VSAPI vsLineMarkerAdd(int wid,int RealLineNum,int isRealLineNum,int NofLines,int BMIndex,int type,const char *pszMessage VSDEFAULT(0), void *pUserData VSDEFAULT(0), void *reserved VSDEFAULT(0));
/**
 ...
 * </pre>
 */
int VSAPI vsStreamMarkerAdd(int wid, long StartOffset, long Length, int isRealOffset, int BMIndex,int type,const char *pszMessage=0,void *pUserData=0,void *reserved=0);
 ...
 */
EXTERN_C
int VSAPI vsStreamMarkerAddB(const char *pszBufName,long StartOffset,long Length,int isRealOffset,int BMIndex,int type,const char *pszMessage, void *pUserData=0,void *reserved=0);
 ...
 * @categories Marker_Functions
 */
vsSeekPos VSAPI vsStreamMarkerGetStartOffset(int StreamMarkerIndex);
 ...
 * @categories Marker_Functions
 */
int VSAPI vsStreamMarkerGetLength(int StreamMarkerIndex);
 ...
 *
 */
int VSAPI vsLineMarkerAddB(const char *pszBufName,int RealLineNum,int isRealLineNum,int NofLines,int BMIndex,int type,const char *pszMessage VSDEFAULT(0), void *pUserData VSDEFAULT(0),const char *pszLineData VSDEFAULT(0), void *reserved VSDEFAULT(0));
/**
 * Adds a new line marker.   Note that using a mark id instead of a line
 * number has the advantage that the number of lines in the file do not
 * need to be calculated.  This is only useful for data files and not source
 ...
 *
 */
int VSAPI vsLineMarkerAddMarkIdB(const char *pszBufName,int markid,int BMIndex,int type,const char *pszMessage VSDEFAULT(0), void *pUserData VSDEFAULT(0), const char *pszLineData VSDEFAULT(0), int RealLineNum VSDEFAULT(0), long BeginLineROffset VSDEFAULT(0),int col VSDEFAULT(0), void *reserved VSDEFAULT(0));
/**
...
int VSAPI vsMarkerTypeSetCallbackMouseEvent(
   int type,
   int event,
   int (VSAPI * pfnCommand)(int wid,int LineNum, int LineMarkerIndex,int BMIndex,int Type, const char *pszMessage, void *pUserData) VSDEFAULT(0)
   );
 ...
 *     int p_line = vsPropGetI(DestWid,VSP_LINE);
 ...
 */
int *VSAPI vsLineMarkerAllocFindList(int wid,int LineNum,long LineOffset=VSNULLSEEK,int CheckNofLines=0,int *pNofItems=0);
 ...
 *         Call vsFree to free the list.
 */
int *VSAPI vsStreamMarkerAllocFindList(int wid,int *pNofItems,long StartOffset,long Length,long SearchStartOffset=VSNULLSEEK,int type=0,int internalStartBlock=-1,long internalStartBlockPos_l=0);
/**
 ...
 * @categories Marker_Functions
 */
void VSAPI vsStreamMarkerSetLength(int StreamMarkerIndex,long Length);
 ...
 * @categories Marker_Functions
 */
void VSAPI vsStreamMarkerSetStartOffset(int StreamMarkerIndex, long StartOffset);
 ...
 * @categories Marker_Functions
 */
void VSAPI vsStreamMarkerSetMessage(int StreamMarkerIndex, char *pszMessage);
 ...
 * @categories Marker_Functions
 */
int VSAPI vsScrollMarkupAdd(int wid,int iLineNum,int iType,int iLength VSDEFAULT(1));
int VSAPI vsScrollMarkupAddOffset(int wid,vsSeekPos offset,int iType,int iLength VSDEFAULT(1));
...
void VSAPI vsScrollMarkupGetMarkup(int iWID,slickedit::SEArray<VSSCROLLBAR_MARKUP_INFO> &markupInfo,
                                   vsSeekPos startOffset VSDEFAULT(0),
                                   vsSeekPos endOffset VSDEFAULT(-1));
...
int VSAPI vsClexGetNextToken(int wid,unsigned int &iTokenLen,
                             long &tokenSeekPos,VSCOLORINDEX &tokenColor,bool bGetFirstToken=false,
                             int skipFlags=0
                             );

EXTERN_C
int VSAPI vsCreateZipFile(VSPSZ pszFilename, VSHREFVAR files, VSHREFVAR zipStatus);
...
typedef slickedit::SEString (*ExtFileContentsCallback)(const slickedit::SEString &buffer, int fileHandle);
